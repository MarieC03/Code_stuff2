...
struct kastaun_c2p_lep4d_t {
    ...
    ) : eos(_eos), metric(_metric), h0(_eos.enthalpy_minimum())
    {
        ...
        ye  = Kokkos::fmin(eos.get_c2p_ye_max(), Kokkos::fmax(eos.get_c2p_ye_min(), ye)) ;
        ymu = Kokkos::fmin(eos.get_c2p_ymu_max(), Kokkos::fmax(eos.get_c2p_ymu_min(), ymu)) ;
...
struct entropy_fix_c2p_lep4d_t {
    ...
    ) : eos(_eos), metric(_metric), h0(_eos.enthalpy_minimum())
    {
        ...
        r_dot_Btilde2 = r_dot_Btilde*r_dot_Btilde ;
        r = metric.raise(r) ;
        ye = Kokkos::fmin(eos.get_c2p_ye_max(),
                          Kokkos::fmax(eos.get_c2p_ye_min(), conservs[LEP4D_YESL] / D));
        ymu = Kokkos::fmin(eos.get_c2p_ymu_max(),
                           Kokkos::fmax(eos.get_c2p_ymu_min(), conservs[LEP4D_YMUSL] / D));
        v02 = r2/(h0*h0 + r2) ;
    }
