...
GRACE_HOST_DEVICE inline constexpr int m1_num_species()
{
...
GRACE_HOST_DEVICE inline constexpr int m1_aux_index(int base_idx, int ispec)
{
    return base_idx + ispec * GRACE_N_M1_AUX;
}

GRACE_HOST_DEVICE inline constexpr int m1_ye_coupling_sign(int ispec)
{
#ifdef M1_NU_FIVESPECIES
    return (ispec == 0) - (ispec == 1);
...
GRACE_HOST_DEVICE inline constexpr int m1_ymu_coupling_sign(int ispec)
{
#ifdef M1_NU_FIVESPECIES
    return (ispec == 2) - (ispec == 3);
#else
    return 0;
#endif
}
...
primsarr[ERADL] = vview(__VA_ARGS__,m1_evolved_index(ERAD_,ispec),q);        \
primsarr[NRADL] = vview(__VA_ARGS__,m1_evolved_index(NRAD_,ispec),q);        \
primsarr[FXL] = vview(__VA_ARGS__,m1_evolved_index(FRADX_,ispec),q) ;        \
primsarr[FYL] = vview(__VA_ARGS__,m1_evolved_index(FRADY_,ispec),q) ;        \
primsarr[FZL] = vview(__VA_ARGS__,m1_evolved_index(FRADZ_,ispec),q) ;        \
