#!/usr/bin/perl -s
BEGIN { $^I = ''; }
if(!($coord||$d||$g||$s||$p||$eos||length($phi)||length($z)||$arch||length($nf))){
   print STDERR <<HELPMSG ;

$0 [-d=NdimNdir] [-g=n1,n2,n3] [-p=PHYSICS] [-eos=EOS] [-phi=Idirphi] [-z=Idirz] \\
       [-nf=nf] [-arch=arch] [-coord=coord] [-s]
...
PHYSICS is the name of the physics module (folder in src).
...
Set the environment variable $BHAC_DIR to the code main directory, e.g. \

export $BHAC_DIR=youramrvacdir

ENVMSG
exit}

$showonly= $s && !($coord||$d||$g||$p||$eos||$arch||length($nf)||length($phi)||length($z));

# Get the makefile:
    $filename="makefile";
    unless (-e($filename)){                                                                        
	print "Getting makefile from source directory\n";
	`cp $ENV{BHAC_DIR}/arch/make_temp $filename`;
    }
...
# Get the definitions.h:
	...
	`cp $ENV{BHAC_DIR}/src/$filename $filename`;
    }
...
unless ($showonly){
    foreach $filename ('amrvacusr.t','amrvacusrpar.t'){
      unless (-e($filename)){
	print STDERR "You need to provide $filename yourself!\n";
      } 
      }
}
...
$settings="-d=$d -phi=$phi -z=$z -g=$g -p=$phys -eos=$eos -nf=$nf -arch=$arch -coord=$coord";
