...
subroutine collapse_subnode(igrid,jgrid,dir,normconv)
 ...
 !-----------------------------------------------------------------------------
call mpistop('passed collapse subnode')
 ...
 if(nwauxio>0)then
    normconv(nw+1:nw+nwauxio)=one
      ! M1_test
      !call specialvar_output(ixG^LL,ixM^LL,nw+nwauxio,w,ps(igrid),normconv, &
 ...
 if(saveprim) then
    !call primitive(ixG^LL,ixG^LL,w(ixG^T,1:nw),px(igrid)%x)
