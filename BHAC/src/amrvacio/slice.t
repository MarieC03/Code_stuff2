...
contains
  ...
  contains
         ...
         do ipe=1,npe-1
            do jgrid=1,Morton_sub_stop(ipe)-Morton_sub_start(ipe)+1
               ...
               if (ipe == mype ) then 
                  ...
                  call MPI_SEND(pxC_sub(jgrid)%x,1,type_subblockC_x_io,0,itag+2,icomm,ierrmpi)
                  call MPI_SEND(pwC_sub(jgrid)%w,1,type_subblockC_io,0,itag+3,icomm,ierrmpi)
               ...
               if (mype == 0) then
                  ...
                  call MPI_RECV(pxC_sub(Njgrid+1)%x,1,type_subblockC_x_io,ipe,itag+2,icomm,status,ierrmpi)
                  call MPI_RECV(pwC_sub(Njgrid+1)%w,1,type_subblockC_io,ipe,itag+3,icomm,status,ierrmpi)
    ...
    subroutine write_slice_vtk(jgrid,slice_fh,wnamei)
      ...
      case('vtu') ! pointdata
         ...
         do iw=1,nw+nwauxio
            ...
            write(slice_fh,'(a,a,a)')&
                 '<DataArray type="Float64" Name="',TRIM(wnamei(iw)),'" format="ascii">'
            write(slice_fh,'(200(1pe14.6))') {^DM&(|}roundoff_minmax(pwC_sub(jgrid)%w(ix^DM,iw)*normconv(iw),minvalue,maxvalue),{ix^DM=ixCmin^DM,ixCmax^DM)}
            write(slice_fh,'(a)')'</DataArray>'
         enddo
         write(slice_fh,'(a)')'</PointData>'
      ...
      {do ix^DMB=ixCmin^DMB,ixCmax^DMB \}
            x_VTK(1:3)=zero;
            x_VTK(1:ndim)=pxC_sub(jgrid)%x(ix^DM,1:ndim)*normconv(0);
            {^IFTHREED
              if (typeaxial == 'slab' .or. typeaxial =='cartesian') then
                if (slice_type=='vtuCC') then
                    aux_ix1 = ix1
                    aux_ix2 = ix2
                    if (ix1==ixCmin1) aux_ix1=ix1+1
                    if (ix2==ixCmin2) aux_ix2=ix2+1
                    x_VTK(dir)=px_sub(jgrid)%x(aux_ix^DM,dir)*normconv(0);
                else
                    x_VTK(dir)=xslice;
                endif
              endif
    ...
    subroutine put_slice_csv
      ...
      if (mype==0) then
         ...
         call getheadernames(wnamei,xandwnamei,outfilehead)
         line=''
         ! distinguis x,y,z and r,theta,phi etc:
      ...
      do ix2=ixsubMlo(2),ixsubMhi(2)
         do ix1=ixsubMlo(1),ixsubMhi(1)
            ...
            do ix1=ixsubMlo(1),ixsubMhi(1)
               ...
               do idir=1,ndim
                  {^IFTHREED
                  write(data,"(es14.6)")roundoff_minmax(px_sub(jout)%x(ix1,ix2,idir),minvalue,maxvalue)
                  }
                  {^IFTWOD
                  write(data,"(es14.6)")roundoff_minmax(px_sub(jout)%x(ix1,idir),minvalue,maxvalue)
                  }
                  {^IFONED
                  write(data,"(es14.6)")roundoff_minmax(px_sub(jout)%x(idir),minvalue,maxvalue)
                  }
  ...
  subroutine fill_subnode(igrid,active,jgrid,dir,xslice,normconv)
    ...
    call calc_grid(unitslice,igrid,xC,xCC,xC_TMP,xCC_TMP,wC_TMP,wCC_TMP,normconv,&
         ixC^L,ixCC^L,.true.,slice_nocartesian)
    ...
    px_sub(jgrid)%x(1:ndim) = xCC_TMP(ixslice,1:ndim)
    pwC_sub(jgrid)%w(1:nw+nwexpand) = wC_TMP(ixslice,1:nw+nwexpand)
    pxC_sub(jgrid)%x(1:ndim) = xC_TMP(ixslice,1:ndim)
    }
    {^IFTWOD
    select case (dir)
    case (1)
       ...
       px_sub(jgrid)%x(ixCCmin2:ixCCmax2,1:ndim) &
            = xCC_TMP(ixslice,ixCCmin2:ixCCmax2,1:ndim)
       pwC_sub(jgrid)%w(ixCmin2:ixCmax2,1:nw+nwexpand) &
            = wC_TMP(ixslice,ixCmin2:ixCmax2,1:nw+nwexpand)
       pxC_sub(jgrid)%x(ixCmin2:ixCmax2,1:ndim) &
    ...
    case (2)
       ...
       px_sub(jgrid)%x(ixCCmin1:ixCCmax1,1:ndim) &
            = xCC_TMP(ixCCmin1:ixCCmax1,ixslice,1:ndim)
       pwC_sub(jgrid)%w(ixCmin1:ixCmax1,1:nw+nwexpand) &
            = wC_TMP(ixCmin1:ixCmax1,ixslice,1:nw+nwexpand)
       pxC_sub(jgrid)%x(ixCmin1:ixCmax1,1:ndim) &
  ...
  subroutine alloc_subnode(jgrid,dir,nwexpand)
    ...
    allocate(pwC_sub(jgrid)%w(1:nw+nwexpand),pxC_sub(jgrid)%x(1:ndim))
    }
    {^IFTWOD
    select case (dir)
    case (1)
       allocate(pw_sub(jgrid)%w(ixGlo2:ixGhi2,1:nw+nwexpand),&
            ...
            pwC_sub(jgrid)%w(ixGlo2:ixGhi2,1:nw+nwexpand),&
            pxC_sub(jgrid)%x(ixGlo2:ixGhi2,1:ndim))
    case (2)
       allocate(pw_sub(jgrid)%w(ixGlo1:ixGhi1,1:nw+nwexpand),&
            px_sub(jgrid)%x(ixGlo1:ixGhi1,1:ndim), &
            pwC_sub(jgrid)%w(ixGlo1:ixGhi1,1:nw+nwexpand),&
            pxC_sub(jgrid)%x(ixGlo1:ixGhi1,1:ndim))
    case default
  ...
  subroutine dealloc_subnode(jgrid)
    ...
    deallocate(pw_sub(jgrid)%w,px_sub(jgrid)%x,pwC_sub(jgrid)%w,pxC_sub(jgrid)%x)
