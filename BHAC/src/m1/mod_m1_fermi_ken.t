module mod_m1_fermi
  ...
  double precision, save :: FII(0:6,NfermiDim)
  logical, save :: FII_initialized = .false.
...
contains
 ...
 ! Initialize the lookup table once
  subroutine initialize_fermi_tables()
    if (.not. FII_initialized) then
      call prepare_fermi(FII)
      FII_initialized = .true.
    end if
  end subroutine initialize_fermi_tables
 ...
 ! uses fermi_calc(..) for eta < 2 and direct_integral(...) for eta >2
  ...
  ! Initialize on first call
    if (.not. FII_initialized) call initialize_fermi_tables()
