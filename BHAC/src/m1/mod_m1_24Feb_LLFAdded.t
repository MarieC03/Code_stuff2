...
subroutine m1_correct_asymptotic_fluxes(ixI^L, ixO^L, tvdlfeps1, dxdim, idims, wLC, wRC, fC, fLC, fRC, cminC, cmaxC, wradimpl, qtC)
  ...
  double precision, intent(in) :: tvdlfeps1, qtC
  double precision, intent(in), dimension(1:ndim)                   :: dxdim
  double precision, intent(in), dimension(ixI^S,1:nwflux)           :: fLC, fRC
  double precision, intent(inout), dimension(ixI^S,1:nwflux,1:ndim) :: fC
  double precision, intent(in), dimension(ixI^S,1:nw)               :: wLC, wRC
  double precision, intent(in), dimension(ixI^S,1:ncons)            :: cminC, cmaxC
  double precision, intent(in), dimension(ixI^S,1:nm1rad_eas)       :: wradimpl

  double precision, dimension(ixO^S) :: A
  double precision, dimension(ixO^S) :: alpha, F_llf, F_cor
  double precision, dimension(ixO^S) :: nfloor
  double precision, dimension(ixO^S) :: Fmin, Fmax
  integer :: iw
  integer :: i, j
  integer :: ixR^L
  {#IFDEF UNIT_TESTS
  ...
  {^KSP&
    ...
    ! -------------------- nrad (POSITIVITY SAFE) --------------------
    iw = nrad^KSP_

    ! Choose a proper number-density floor (do NOT use m1_E_atmo).
    ! If you don't have m1_N_atmo yet, define it in mod_m1 as a parameter.
    nfloor(ixO^S) = m1_E_atmo   ! <-- you should introduce this

    ! Rusanov/LLF speed
    alpha(ixO^S) = MAX(DABS(cminC(ixO^S,iw)), DABS(cmaxC(ixO^S,iw)))

    ! Safe monotone LLF flux
    F_llf(ixO^S) = 0.5d0*(fLC(ixO^S,iw) + fRC(ixO^S,iw)) - 0.5d0*alpha(ixO^S)*(wRC(ixO^S,iw) - wLC(ixO^S,iw))

    ! Candidate asymptotic-corrected HLL flux (your original form)
    ...
    ! --- Positivity guard ---
    ! If either reconstructed state is near the floor, force LLF.
    where( (wLC(ixO^S,iw) <= 2.0d0*nfloor(ixO^S)) .or. (wRC(ixO^S,iw) <= 2.0d0*nfloor(ixO^S)) )
      fC(ixO^S,iw,idims) = F_llf(ixO^S)
    elsewhere
      ! Otherwise, allow correction but CLIP it around LLF to prevent anti-diffusive spikes.
      ! This keeps f within a conservative envelope centered on the monotone flux.
      !
      ! Envelope size: proportional to LLF dissipation term.
