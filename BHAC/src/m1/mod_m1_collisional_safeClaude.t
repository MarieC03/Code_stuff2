...
contains
  ...
  subroutine m1_get_implicit_collisional_sources(wrad, metricM1, vel_, eas_,ixI^L,ix^D, N, sources, qdt, speciesKSP, dtfactor)
   ...
   ! Ut_explicit(1) = wrad(m1_energy_)/ metricM1%sqrtg(ix^D)
    ...
    !------------------------for singularieties !KEN commented this out
    ...
    if(linearized_eqs) then
      ...
      call rootfinding_global_multid_newton_raphson(m1_system_ndim, wrad, 1.0d-15, 150, &
    ...
    ! -----------------------------------------------------------------------
    ! ADMISSIBILITY ENFORCEMENT: guarantee F^2 < E^2 after the implicit solve.
    ! The Newton-Raphson may have wandered into a region where this is violated
    ! (especially when the Jacobian evaluation was stale).  A violated state
    ! produces NaNs / infinities in BThick and the subsequent con2prim.
    ! We raise F_low to F_up using the diagonal metric shortcut available here
    ! (metricM1 is in scope), then rescale if necessary.
    ! -----------------------------------------------------------------------
    stateM1%F_up(1) = metricM1%gammaUPij(ix^D,1)*stateM1%F_low(1) &
                    + metricM1%gammaUPij(ix^D,2)*stateM1%F_low(2) &
                    + metricM1%gammaUPij(ix^D,3)*stateM1%F_low(3)
    stateM1%F_up(2) = metricM1%gammaUPij(ix^D,2)*stateM1%F_low(1) &
                    + metricM1%gammaUPij(ix^D,4)*stateM1%F_low(2) &
                    + metricM1%gammaUPij(ix^D,5)*stateM1%F_low(3)
    stateM1%F_up(3) = metricM1%gammaUPij(ix^D,3)*stateM1%F_low(1) &
                    + metricM1%gammaUPij(ix^D,5)*stateM1%F_low(2) &
                    + metricM1%gammaUPij(ix^D,6)*stateM1%F_low(3)
    Fmag = {^C& stateM1%F_low(^C)*stateM1%F_up(^C)+}
    if (.not. ieee_is_finite(Fmag)) Fmag = 0.0d0
    Fmag = max(Fmag, 0.0d0)
    if (Fmag > (fmaxfact*stateM1%E)**2) then
      fac = fmaxfact*stateM1%E / dsqrt(Fmag)
  ...
  function m1_collisional_jacobian(z_vec)
    ...
    ! Always update stateM1 from z_vec and re-run the closure so that the
    ! Jacobian is evaluated at the same point as m1_collisional_sources.
    ! Without this the Newton-Raphson uses a stale Jacobian which can produce
    ! iterates that violate F^2 < E^2, ultimately causing NaNs in con2prim.
    stateM1%E = z_vec(1)
  ...
  function m1_sources_func(z_vec)
   ...
   !  G = coll_sources = alp*sqrtg*G_bar 
    ...
    m1_sources_func_global(:) = m1_sources_func(:)
  ...
  function m1_sources_min(z_vec)
    ...
    Fsource_low(:) = m1_sources_func_global(:) !KEN
