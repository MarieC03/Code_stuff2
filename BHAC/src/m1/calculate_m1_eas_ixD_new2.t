 ...
 !****************************************************************************
  subroutine calculate_m1_eas_ixD(wrad,N,ix^D,Gamma,vel,Wlor,Jrad,ixI^L,x,eas, &
    ...
    if(m1_rates_Weakhub) then
        ...
        !>  (same Blackbody-based code-unit form as in the old mod_m1_eas.t)
        !>  Q_code = kappa_a_code * Black_er,   R_code = kappa_n_code * Black_nr
        Qems   = max(eas_eq(k_a) * Black_er, 1.0d-60)
        Qems_n = max(eas_eq(k_n) * Black_nr, 1.0d-60)
        ...
        if (speciesKSP == m1_i_nux .or. speciesKSP == m1_i_mu .or. &
          ...
          call m1_eas_analytic_plasmon(speciesKSP, eta_rad, eta_e, &
               ye_fluid, T_fluid, rho_fluid, eas, Qems, Qems_n_sec)
          ...
          Qems_sec   = Qems_sec   + Qems
          Qems_n_sec = Qems_n_sec + Qems_n
          !> A4 – Kirchhoff kappa from secondary emission, using the same
          !> blackbody normalization as in the old mod_m1_eas.t.
          Qems_sec   = max(Qems_sec   * MEV_TO_ERG_KEN * RHOGF * EPSGF / TIMEGF, 1.0d-60)
          Qems_n_sec = max(Qems_n_sec * MNUC_CGS * RHOGF / TIMEGF,               1.0d-60)

          kappa_a_K = 0.0d0
          kappa_n_K = 0.0d0
          if (Qems_sec   > 0.0d0 .and. ieee_is_finite(Qems_sec))   &
               kappa_a_K = Qems_sec   / (Black_er + 1.0d-40)
          if (Qems_n_sec > 0.0d0 .and. ieee_is_finite(Qems_n_sec)) &
               kappa_n_K = Qems_n_sec / (Black_nr + 1.0d-40)

          !> A5 – accumulate Kirchhoff kappas onto Weakhub kappas in code units
          eas_eq(k_a) = eas_eq(k_a) + kappa_a_K
          eas_eq(k_n) = eas_eq(k_n) + kappa_n_K

          !> Recompute total emission including secondary processes in code units
          Qems   = max(eas_eq(k_a) * Black_er, 1.0d-60)
          Qems_n = max(eas_eq(k_n) * Black_nr, 1.0d-60)
        end if
     ...
     !=== end #ifdef M1_RATES (Path A) =====================================
        ...
        ! Reuse blackbody_ixD result only here, where the analytical path works
        ! explicitly with CGS/MeV Kirchhoff factors.
        BB_n_mev = max(Black_nr * CLITE_CM * C2_CGS / HC_MEVCM**2, 1.0d-60)
        BB_e_mev = max(Black_er * CLITE_CM * C2_CGS / HC_MEVCM**2, 1.0d-60)
        ...
        ! ======================================================================
        !  Scattering opacity (neutral-current, analytical path only)
        !
        !  Ruffert A1, A8 (1996):
