module mod_m1_fermi
  ...
  double precision, parameter :: ETA_MIN = 1.0d-3 !not necessary or really used
...
contains
   ...
   double precision :: eta_set
      ...
      if( N .eq. 0 ) then
       if ( eta > ETA_MIN) then
         ...
         !eta_set = ETA_MIN !test
         eta_set = eta
	      fermi_dirac = fermi_calc(0,eta_set,FII) 
       end if
    else if ( N .eq. 1 ) then 
       if ( eta > ETA_MIN ) then
         ...
         !eta_set = ETA_MIN !test
         eta_set = eta
	      fermi_dirac = fermi_calc(1,eta_set,FII)
       end if
    else
         if(eta > ETA_MIN ) then
	         fermi_dirac = fermi_calc(N,eta,FII)
         else
            write(76,*)"eta small",eta
            !eta_set = ETA_MIN !test
            eta_set = eta
            fermi_dirac = fermi_calc(N,eta_set,FII)
         end if
 ...
 !===============================================================
 !=============================================================== 
  subroutine get_neutrino_temp_ixD(wrad,N,ix^D,Gamma,velU,Wlor,Jrad, eta, speciesKSP, fluid_Prim, av_energy, T_nu, timeCurrent1,x)    
   ...
   !write(93,*) "---------- ixd",ix^D
   !! if(timeCurrent1>0.0) then
   !!    write(92,*) "---------- ixd",ix^D
   !!    if(x(1) .ge. 4.0d0 .and. x(1) .le. 6.0d0 .and. x(2) .le. 1.0d0) then
   !!       write(91,*)"---------- ixd",ix^D
   !!    end if 
   !! endif
     ...
     !write(93,*) "in fermimod, E,n,N", E,n_dens,N
     !! if(timeCurrent1>0.0) then
     !!  write(92,*) "in fermimod, E,n,N", E,n_dens,N
     !!  if(x(1) .ge. 4.0d0 .and. x(1) .le. 6.0d0 .and. x(2) .le. 1.0d0) then
     !!     write(91,*)"in fermimod, E,n,N", E,n_dens,N
     !!     write(91,*)"in fermimod, E/N", E/N
     !!  end if 
     !! endif
     ...
     if((E .eq. 0.d0) .and. (Fv .eq. 0.d0)) then
        ...
        if(n_dens < 1.d-16 .or. E<1.d-16) then
           n_dens = 1.d-16
           E = 1.d-16
        endif
        ...
        T_nu = fermi_dirac(eta,2)/fermi_dirac(eta,3)*av_energy_MEV

        !write(93,*) "in fermimod, T_nu", T_nu
        !!if(timeCurrent1>0.0) then
        !! write(92,*) "in fermimod, T_nu", T_nu
        !! if(x(1) .ge. 4.0d0 .and. x(1) .le. 6.0d0 .and. x(2) .le. 1.0d0) then
        !!    write(91,*) "in fermimod, T_nu", T_nu
        !! end if 
        !!endif
       ...
       ! write(93,*) "in fermimod, E,n set", E,n_dens
       ! write(93,*) "in fermimod, n/E, E/n", n_dens/E, E/n_dens
       ! write(93,*) "in fermimod, f2/f3", fermi_dirac(eta,2)/fermi_dirac(eta,3)
       ! write(93,*) "in fermimod, av_MEV", av_energy_MEV
       ! write(93,*) "in fermimod, T_nu set", T_nu

       !! if(timeCurrent1>0.0) then
       !!  write(92,*) "in fermimod, E,n set", E,n_dens
       !!  write(92,*) "in fermimod, n/E, E/n", n_dens/E, E/n_dens
       !!  write(92,*) "in fermimod, eta", eta
       !!  write(92,*) "in fermimod, f2/f3", fermi_dirac(eta,2)/fermi_dirac(eta,3)
       !!  write(92,*) "in fermimod, E-Fv", E-Fv
       !!  write(92,*) "in fermimod, av1/Wlor", av_energy1/Wlor
       !!  write(92,*) "in fermimod, av1", av_energy1
       !!  write(92,*) "in fermimod, av_MEV", av_energy_MEV
       !!  write(92,*) "in fermimod, T_nu set", T_nu
       !!  if(x(1) .ge. 4.0d0 .and. x(1) .le. 6.0d0 .and. x(2) .le. 1.0d0) then
       !!     write(91,*) "in fermimod, E,n set", E,n_dens
       !!     write(91,*) "in fermimod, n/E, E/n", n_dens/E, E/n_dens
       !!     write(91,*) "in fermimod, E/N",  E/N
       !!     write(91,*) "in fermimod, E-Fv", E-Fv
       !!     write(91,*) "in fermimod, av1/Wlor", av_energy1/Wlor
       !!     write(91,*) "in fermimod, av1", av_energy1
       !!     write(91,*) "in fermimod, av_MEV", av_energy_MEV
       !!     write(91,*) "in fermimod, eta", eta
       !!     write(91,*) "in fermimod, f2/f3", fermi_dirac(eta,2)/fermi_dirac(eta,3)
       !!     write(91,*) "in fermimod, T_nu set", T_nu
       !!  end if          
       !! endif
 ...
 subroutine blackbody_ixD(wrad,ix^D,speciesKSP,fluid_Prim,T_fluid,eta,Black_er,Black_nr)
      ...
      if(m1_use_muons) then
       ...
       Black_nr = Black_nr_MEV * MEV_TO_ERG * EPSGF * RHOGF/TIMEGF *LENGTHGF 
