...
! ====================================================================== 
    ...
    subroutine fill_metric(metricH,wprim,x,ixI^L,ixO^L, get_deriv)
        ...
        double precision, intent(in)    :: wprim(ixI^S,1:nw) !KEN changed to inout to in
        ...
        ! ======================================================================
        ! 1) sqrtgamma_hat 
        call get_sqrt_gamma_hat(x(ixI^S, 1:^ND), ixI^L, ixO^L, sqrtgHat(ixI^S))
        ! ======================================================================
        ! Recover physical sqrtg = sqrtg_hat * pow(psi,-4) 
        ! ======================================================================

        !KEN did this to avoid expensive **
        !KEN only does ixO^S to "save" resources
        psi4(ixO^S)    = wprim(ixO^S, psi_metric_)**4
        psi6(ixO^S)   = psi4(ixO^S) * wprim(ixO^S, psi_metric_)**2
        psi4inv(ixO^S) = 1.0d0 / psi4(ixO^S)
