...
call set_tmax_particles

do while (t_particles .lt. tmax_particles)

   ! set the synchronization time   
   call set_tsync_particles
   
   ! add new particles (user defined)
   call add_particles

   ! integrate to tsync_particles
...
double precision                :: t_min_mype, dt_particles_mype
!-----------------------------------------------------------------------------
...
do while (mynparticles .lt. nparticles)
   ...
   if (nparticles_on_mype .ge. nparticles_per_cpu_hi) then
      call mpistop('read_particles_snapshot: max number of particles per cpu reached, increase nparticles_per_cpu_hi!')
   end if
