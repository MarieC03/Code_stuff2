...
{#IFDEF M1
   if (allocated(fC_low) .and. m1_fallback_flux) then
      ...
      {^KSP&
         ...
         if (slab) then
            need_fallback(ixO^S) = ( wnew(ixO^S,nrad^KSP_) + &
                 dxinv(idims)*( fC(ixO^S,nrad^KSP_,idims)-fC(hxO^S,nrad^KSP_,idims) ) < m1_E_atmo )
         else
            select case (idims)
            {case (^D)
               need_fallback(ixO^S) = ( wnew(ixO^S,nrad^KSP_) + &
                    (-qdt)*( fC(ixO^S,nrad^KSP_,^D)-fC(hxO^S,nrad^KSP_,^D) )/mygeo%dvolume(ixO^S) < m1_E_atmo )
            \}
            end select
         end if
