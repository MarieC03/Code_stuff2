...
if (mype == 0) then 
   ...
   {do ix^DB=1,nc^DB\}  
     ...
     case('c2b_phiav')
        read(unit,*) xoneblock(ix^D,1:^ND), woneblock(ix^D,rho_),woneblock(ix^D,u1_),woneblock(ix^D,u2_),&
                                            woneblock(ix^D,u3_)
     case('c2b_phiav_tabulated')
        read(unit,*) xoneblock(ix^D,1:^ND), woneblock(ix^D,rho_),woneblock(ix^D, ye_), woneblock(ix^D,T_eps_), dummy,&
                              woneblock(ix^D,v1_), woneblock(ix^D, v2_), woneblock(ix^D,v3_)
     case('')
