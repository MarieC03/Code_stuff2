...
module mod_amr_fct
  ...
  type(facealloc), dimension(2,^ND,ngridshi), save :: pface

  type(fake_neighbors), dimension(2^D&,^ND,ngridshi), save :: fine_neighbors

  integer, dimension(2,-1:1^D,ngridshi), save :: old_neighbor
...
integer :: ixCosV^L(1:^ND),ixFisV^L(1:^ND)
...
double precision :: bfluxCo(sCo%ws%ixG^S,nws),bfluxFi(sFi%ws%ixG^S,nws)
double precision :: slopes(sCo%ws%ixG^S,ndim)
{^IFTHREED
...
if (ghost) then
ixFi^L=ixFi^Lin;
{ixComin^D=int((ixFimin^D+dixB+1)/2);}
{ixComax^D=int((ixFimax^D+dixB+1)/2);}
else
ixCo^L=ixCo^Lin;
ixFi^L=ixM^LL;
end if
...
ixGsmin^D=sFi%ws%ixGmin^D;
ixGsmax^D=sFi%ws%ixGmax^D;
! -------------------------
...
do idim1=1,ndim
  ...
  if (ghost) then
    ...
    ixCosE^L=int((ixFisE^L+dixB+1)/2);
  else
  ...
  select case(idim1)
 {case(^D)
  bfluxFi(ixFisE^S,idim1)=wFis(ixFisE^S,^D)*sFi%geo%surfaceC^D(ixFisE^S)
 \}
  end select
 ...
 {do ix^DE=0,1\}
  ...
  {^IFTHREEDixFisC^L=ixFisC^L+ix3*kr(idim3,^D);}

  bfluxCo(ixCosE^S,idim1)=&
  bfluxCo(ixCosE^S,idim1)&
...
do idim1=1,ndim
  ...
  select case(idim1)
 {case(^D)
      bfluxCo(ixCosE^S,^D)=wCos(ixCosE^S,^D)&
      *sCo%geo%surfaceC^D(ixCosE^S)
\}
  end select
...
do idim1=1,ndim
   ...
   do idim2=1,ndim
     ...
     slopes(ixCos^S,idim2)=&
       0.125d0*(bfluxCo(jxCos^S,idim1)-bfluxCo(hxCos^S,idim1))
{^IFTWOD
     do ix2=0,1
       ixFisCmin^D=ixFisVmin^D(idim1)+ix2*kr(^D,idim2);
       ixFisCmax^D=ixFisVmax^D(idim1)+ix2*kr(^D,idim2);
       bfluxFi(ixFisCmin^D:ixFisCmax^D:2,idim1)=&
        half*(bfluxCo(ixCos^S,idim1)&
...
!       Toth-Roe
        alpha(ixCo^S,idim1)=(sCo%geo%dx(ixCo^S,idim2)-sCo%geo%dx(ixCo^S,idim3))/(sCo%geo%dx(ixCo^S,idim2)+sCo%geo%dx(ixCo^S,idim3))
    end do
  end do
end do

!call printarray(ixCo^L,ixCo^L,alpha(ixCo^S,1))
!call printarray(ixCo^L,ixCo^L,alpha(ixCo^S,2))
!call printarray(ixCo^L,ixCo^L,alpha(ixCo^S,3))
}
...
do idim1=1,ndim
   ...
   select case(idim1)
   {case(^D)
    where(sFi%geo%surfaceC^D(ixFisC^S).ne.zero)
      wFis(ixFisC^S,^D)=bfluxFi(ixFisC^S,^D)/sFi%geo%surfaceC^D(ixFisC^S)
    elsewhere
...
{^IFPHI
    ...
    if (neighbor_type(i^DD,igrid).eq.2) then
      ...
      if (refine(ineighbor,ipe_neighbor)) then
       ! print *, 'My neighbor will be refined! igrig, ipe',ineighbor,ipe_neighbor
        ...
        pface(iside,^D,igrid)%face(1^D%1:nx^DD)=&
          ps(igrid)%ws%w(ixMlo^D-1^D%ixM^T,^D)
        else !! right side
        pface(iside,^D,igrid)%face(1^D%1:nx^DD)=&
          ps(igrid)%ws%w(ixMhi^D^D%ixM^T,^D)
        end if
       ! print *, 'allocated iside, idims, igrid',iside,^D,igrid

        if (ipe_neighbor.ne.mype) nsend_fc(^D)=nsend_fc(^D)+1
...
{^IFPHI if (neighbor_pole(i^D,igrid)/=0) cycle}
        if (neighbor_type(i^D,igrid).eq.4) then
         {do ic^DB=1+int((1+i^DB)/2),2-int((1-i^DB)/2)
          inc^DB=ic^DB+i^DB\}
           ...
           ! print *, 'icD,ineighbor',ic^D
           ! print *, 'incD,ineighbor',inc^D

           ! print *, 'ineighbor',fine_neighbors(ic^D,idims,igrid)%igrid
!            print *, 'ipe_neigh',fine_neighbors(ic^D,idims,igrid)%ipe
         {end do\}
        end if
      end do
    end do
...
do iigrid=1,igridstail; igrid=igrids(iigrid);
 {do iside=1,2
    if (associated(pface(iside,^D,igrid)%face)) then
     ...
     ! print *, 'deallocated iside, idims, igrid',iside,^D,igrid
    end if
  end do\}
end do
...
! Then plug it into the structure pfaces and get the faces 
  ...
  if (ineighbor.gt.0) then
    if (iside.eq.1) then ! left side
     ...
     !   print *, 'iside:',iside,'recv. from coarse at: ',tree%node%ig^DD
     ! end if
     ! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
      if (ipe_neighbor.eq.mype) then
        sFi%ws%w(ixMlo^D-1^D%ixM^T,^D)=pface(iotherside,^D,ineighbor)%face(1^D%1:nx^DD)
     ! !!!!!!!!!!!!!!!!!!!!!!!!!!!!1111
     ! if (mype.eq.0) print *, pface(iotherside,^D,ineighbor)%face(1^D%1:nx^DD)
     ! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      else
     ! print *, 'ibuf',ibuf_recv
        ibufnext=ibuf_recv+isize(^D)
        sFi%ws%w(ixMlo^D-1^D%ixM^T,^D)=reshape(&
             source=recvbuffer(ibuf_recv:ibufnext-1),&
             shape=shape(sFi%ws%w(ixMlo^D-1^D%ixM^T,^D)))
     ! !!!!!!!!!!!!!!!!!!!!!!!!!!!!1111
     ...
     ! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
      if (ipe_neighbor.eq.mype) then
        sFi%ws%w(ixMhi^D^D%ixM^T,^D)=pface(iotherside,^D,ineighbor)%face(1^D%1:nx^DD)
     ! !!!!!!!!!!!!!!!!!!!!!!!!!!!!1111
     ! if (mype.eq.0) print *, pface(iotherside,^D,ineighbor)%face(1^D%1:nx^DD)
     ! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      else
     ! print *, 'ibuf',ibuf_recv
        ibufnext=ibuf_recv+isize(^D)
        sFi%ws%w(ixMhi^D^D%ixM^T,^D)=reshape(&
             source=recvbuffer(ibuf_recv:ibufnext-1),&
             shape=shape(sFi%ws%w(ixMhi^D^D%ixM^T,^D)))
     ! !!!!!!!!!!!!!!!!!!!!!!!!!!!!1111
     ! if (mype.eq.0) print *, recvbuffer(ibuf_recv:ibufnext-1)
     ! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ibuf_recv=ibufnext
