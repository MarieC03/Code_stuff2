...
subroutine faces2centers(ixO^L,s)
  ...
  case (4) 
    ...
    do idim=1,ndim
      ...
      select case(idim)
         {case(^D)
         w(ixO^S,b0_+idim)=(0.0625d0 * mygeo%dx(ixO^S,idim)/mygeo%dvolume(ixO^S) ) &
         !w(ixO^S,b0_+idim)=(0.0625d0/mygeo%surface^D(ixO^S)) &
              *( &
  ...
  case default
    call mpistop('There is no such an order of face_to_center when using CT (staggered), 2 or 4!')
  end select
...
subroutine updatefacescontact(ixI^L,ixO^L,qdt,vnorm,fC,fE,wprim,s)
  ...
  double precision                   :: fC_local(ixI^S,1:nwflux,1:ndim)
  ...
  double precision                   :: lfac(ixI^S), dxidir, alp_prime(ixI^S)
  double precision                   :: circ(ixI^S,1:ndim)
  ...
  integer                            :: hxC^L,ixC^L,jxC^L,ixA^L,ixB^L, ixCp^L
  integer                            :: ixGs^L
  integer                            :: idim,idim1,idim2,idir,iwdim1,iwdim2,iw

  associate(bfaces=>s%ws%w,x=>s%x%x,mygeo=>s%geo)

  call mpistop('UCT CONTACT still cannot be used, it will lower 10 times of the Bfield strength, fix it!')
  ...
  ! fC Does not include surfaceC
  fC_local = fC

  {#IFDEF DY_SP
    call dysp_get_lfac(ixI^L, ixI^L, wprim, x, lfac)

    ! cell center v_hat
    {^C& v_hat(ixI^S,^C) = alp_prime(ixI^S) * wprim(ixI^S, u^C_) / lfac(ixI^S) &
  ...
  \}
    ...
    (fC_local(ixC^S,iwdim1,idim2)+fC_local(jxC^S,iwdim1,idim2)&
    -fC_local(ixC^S,iwdim2,idim1)-fC_local(hxC^S,iwdim2,idim1))

    ! add slope in idim2 direction from equation (50)
    ixAmin^D=ixCmin^D;
    ixAmax^D=ixCmax^D+kr(idim1,^D);
    EL(ixA^S)=fC_local(ixA^S,iwdim1,idim2)-ECC(ixA^S,idir)
    hxC^L=ixA^L+kr(idim2,^D);
    ER(ixA^S)=fC_local(ixA^S,iwdim1,idim2)-ECC(hxC^S,idir)
    ...
    EL(ixA^S)=-fC_local(ixA^S,iwdim2,idim1)-ECC(ixA^S,idir)
    hxC^L=ixA^L+kr(idim1,^D);
    ER(ixA^S)=-fC_local(ixA^S,iwdim2,idim1)-ECC(hxC^S,idir)
...
{^NOONED
  ...
  \}
    ...
    call rec(ixGs^L,ixC^L,idim2,bfaces(ixGs^S,idim1),&
             btilL(ixGs^S,idim1),btilR(ixGs^S,idim1))
    call rec(ixGs^L,ixC^L,idim1,bfaces(ixGs^S,idim2),&
             btilL(ixGs^S,idim2),btilR(ixGs^S,idim2))

    ! fixme: use which one? ixI^L or ixGs^L?
    ...
    {#IFDEF DY_SP
      fE(ixC^S,idir) = qdt * dxidir * sqrt_gamma_hat(ixC^S) * fE(ixC^S,idir)
    }
    {#IFNDEF DY_SP
      fE(ixC^S,idir) = qdt * dxidir * sqrtg(ixC^S) * fE(ixC^S,idir)
    }
...
!=============================================================================
!fixme: fix the avoiding NaN in uct1 just like uct2
...
do idim1=1,ndim
   ...
   {case(^D)
     where(mygeo%surfaceC^D(ixC^S) .gt. 1.0d-9*mygeo%dvolume(ixC^S))
        circ(ixC^S,idim1)=circ(ixC^S,idim1)&
                   /mygeo%surfaceC^D(ixC^S)
      !fixme: so here, already considered the psi6 for Bs?
     elsewhere
        circ(ixC^S,idim1)=zero
     end where
   \}
