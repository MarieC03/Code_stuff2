...
!  public
  ...
  subroutine cfc_solve_beta(solve_beta, Aij)
    ...
    if (solve_beta) then 
        phi_name = 'beta'
   {^C& nvec(^C) = beta_metric^C_  \}
    else
        phi_name = 'X'
   {^C& nvec(^C) = Xvec_metric^C_  \}
    end if
    ...
    case (cartesian)
       do nb = 1, 2*ndim
        {^C& 
             select case (typeB(beta_metric^C_, nb))
             case ('symm')
                ! inner boundary, but not reflecting:
                mg%bc(nb, mg_vec_iphi(^C))%bc_type = mg_bc_neumann
                mg%bc(nb, mg_vec_iphi(^C))%bc_value = 0.0d0
             case ('asymm')
                ! reflecting inner boundary:
                ! fixme: very weird behavior for extremely low res or tightly patched mesh
                mg%bc(nb, mg_vec_iphi(^C))%bc_type = mg_bc_dirichlet
                mg%bc(nb, mg_vec_iphi(^C))%bc_value = 0.0d0
             case default
                ! outer boundary:
                if (cfc_beta_robin_bc) then
                   mg%bc(nb, mg_vec_iphi(^C))%bc_type = mg_bc_robin
                   mg%bc(nb, mg_vec_iphi(^C))%bc_value = 2.0d0
                else
                   mg%bc(nb, mg_vec_iphi(^C))%bc_type = mg_bc_dirichlet
                   mg%bc(nb, mg_vec_iphi(^C))%bc_value = 0.0d0
                end if
    ...
    do iigrid = 1, igridstail
       ...
       call cfc_beta_get_rhs(pw(igrid)%w, px(igrid)%x, ixG^LL, ixM^LL, &
              Aij(ixG^T,1:3,1:3,iigrid), rhs(ixG^T,1:3), solve_beta)
       call get_natural2orthonormal(px(igrid)%x(ixG^T,1:ndim),ixG^LL,ix^L,n2r(ixG^T,1:3))
       ...
       do idir = 1, ndir
          mg%boxes(id)%cc({:,}, mg_vec_iphi(idir)) = pw(igrid)%w(ix^S, nvec(idir)) * n2r(ix^S, idir)
       end do

       ! if comment this, oscillation modes are wrong
    ...
    do iigrid = 1, igridstail
       ...
       do idir = 1, ndir
          pw(igrid)%w(ix^S, nvec(idir)) = mg%boxes(id)%cc({:,}, mg_vec_iphi(idir)) / (n2r(ix^S, idir))
       end do
    end do

  end subroutine cfc_solve_beta

  subroutine cfc_beta_get_rhs(w, x, ixI^L, ixO^L, Aij, rhs, solve_beta)
    ...
    double precision                :: d_alp_over_psi6(ixI^S,1:^ND)


    call imhd_get_tilde_S_i(ixI^L,ixO^L,w,x,rhs(ixI^S,1:3))
    call get_gamma_ij_hat(x(ixI^S,1:ndim),ixI^L,ixO^L,fij(ixI^S,1:3,1:3))

    ! this is 8 pi f^{ij}S_j
    {^C&   rhs(ixO^S, ^C) = 8.0d0 * dpi * rhs(ixO^S, ^C) / fij(ixO^S, ^C, ^C)  \}
    ...
    if (use_h00_coupling .and. use_h00_cfc_betai) then
      alp_over_psi6(ix^S) =  dsqrt(w(ix^S, alp_metric_)**2 - w(ix^S,h_00_) ) / w(ix^S, psi_metric_)**6
    else
      alp_over_psi6(ix^S) = w(ix^S, alp_metric_) / w(ix^S, psi_metric_)**6
    endif
    ...
    do idir = 1, ndim
       call partial_d(alp_over_psi6(ixI^S), ixI^L, ixO^L, x(ixI^S,1:ndim), idir, d_alp_over_psi6(ixI^S,idir))
    end do
