...
!================================================================================
  ...
  character*20, parameter              :: coord="rzks"
...
!=============================================================================
  ...
  subroutine dilaton3()
    ...
    ! up to a3=0, b3=0.
    ! Parameter eqpar(b_) is the dilaton parameter
    ! Assumes eqpar(m_) (ADM-mass) and eqpar(b_) has been set before!  
    ! .. local ..
    ...
    mu   = eqpar(m_) - eqpar(b_)
    tmp  = 1.0d0 + eqpar(b_)/(2.0d0*mu)

    
    coord_epsilon = sqrt(1.0d0 + eqpar(b_)/mu) - 1.0d0
    coord_r0 = 2.0d0*eqpar(m_) / (1.0d0 + coord_epsilon)

    
    coord_a(0) = eqpar(b_)/(2.0d0*mu)
    coord_b(0) = 0.0d0

    
    coord_a(1) = 2.0d0 * sqrt(1.0d0 + eqpar(b_)/mu) + 1.0d0/tmp  &
                 - 3.0d0 - coord_a(0)
    coord_b(1) = sqrt(1.0d0 + eqpar(b_)/mu)/tmp - 1.0d0

    
    coord_a(2) = sqrt(1.0d0 + eqpar(b_)/mu) - 0.5d0 - (coord_a(0))**2 &
         + coord_a(0) * (sqrt(1.0d0 + eqpar(b_)/mu) - 1.0d0)
    coord_a(2) = coord_a(2) / tmp**2

    coord_b(2) = sqrt(1.0d0 + eqpar(b_)/mu) / tmp &
                 - 1.0d0 - ( eqpar(b_)/(eqpar(b_)+2.0d0*mu) )**2
  ...
  subroutine get_sqrtgamma_analytic(x^D,sqrtgamma,is_analytic)
    ...
    sqrtgamma =  grrb({^D& cmplx(x^D, 0.0d0, kind(1.d0))|,})
    
    {^IFZIN
    sqrtgamma = sqrt(x1**4*sin(x^Z)**2)*sqrtgamma
    }{^IFZOUT
    sqrtgamma = x1**2*sqrtgamma
    }
    
  contains
    !=============================================================================
    double complex function grrb(x^D)
    double complex, intent(in)          :: x^D
      !-----------------------------------------------------------------------------

      grrb = sqrt(2.0d0 - N2(xcompact(x1)))
  ...
  contains
    ...
    double complex function alpha(x^D)
      ...
      alpha = B(xcompact(x1)) /sqrt(2.0d0-N2(xcompact(x1))) 
...
!    
  ...
  contains
    ...
    double complex function betar(x^D)
      ...
      betar = sqrt( B(xcompact(x1))**2-( 2.0d0-N2(xcompact(x1)) )*N2(xcompact(x1)) ) / ( 2.0d0-N2(xcompact(x1)) )
...
!    outerhorizon = 0.0d0
!    outerhorizon = coord_r0
    outerhorizon = 2.0d0*(eqpar(m_)-eqpar(b_))
