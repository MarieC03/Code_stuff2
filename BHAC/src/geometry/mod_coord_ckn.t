...
!================================================================================
  ...
  !=============================================================================
  ! Metric components for Cartesian Kerr-Newman coordinates -coord=ckn
  !
  ! Based on the module for Cartesian Kerr-Schild coordinates,
  ! adding charge squared in helper function tmr, and in routine for sqrtgamma
  !
  ! The charge is not coupled to the electromagnetic charge, and is instead
  ! suitable to simulate a 'dark charge' that affects only spacetime.
  !
  ! Hector Olivares & Bart Ripperda
  ! 05.12.2018
  !=============================================================================
  ...
  character*20, parameter              :: coord="ckn"
  ...
  integer, parameter                   :: rexc_= 1, rcut_=2, q_=3   ! Excision radius
  integer, parameter                   :: ncoordpar = 3
...
!=============================================================================
  ...
  subroutine get_helpers(x,y,z,r,r2,tmr,sig,del,T1,T2,isig,idel)
    ...
    r2  = r*r
    tmr = 2.D0*eqpar(m_)*r - coordpar(q_)**2
...
!    else
       ...
       if (r.lt.eqpar(m_)) then
         sqrtgamma = sqrt(1.0d0+(1.4142135623730950d0*(eqpar(m_)-coordpar(q_)**2/r)*Sqrt(rho+&
       ...
       sqrtgamma = sqrt(1.0d0 + 2.0d0*(eqpar(m_)-coordpar(q_)**2/r)/sqrt(rho))
       }
...
!    end if
  ...
  !=============================================================================
  subroutine u4CoordToBL(ixI^L,ixO^L,xCKS,u4CKS,u4BL,J)
