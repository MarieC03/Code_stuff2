...
character*20, parameter     :: coord="numctob"
...
!  logicals: 14 :
   ...
   if (mype .ne. 0) then 
      ...
      !print *, ndim_met,ngg, 'ndim_met, ngg'
...
!============================================================================
  function centered_difference(x, component_, direction_)
    ...
    integer                             :: component_
    integer                             :: direction_
    double precision                    :: centered_difference
...
!-----------------------------------------------------------------------------
    if (direction_ > ^ND) then  ! we assume symmetry in that direction
      centered_difference = zero
    else 
      delta=dx(direction_, levmax)
      x(direction_) = x(direction_) + delta
      call interpolate_metric(x,component_,upper)
      x(direction_) = x(direction_) - 2*delta
      call interpolate_metric(x,component_,lower)
      centered_difference = (upper - lower) / (2*delta)
    end if 
...
!=============================================================================
      ...
      if (.not.present(jdir)) then
        call interpolate_metric(x,alpha_,alpha) 
          ...
          if (present(dalphadj)) then
            dalphadj = centered_difference(x, alpha_, jdir)
                    ! if (islog.and. jdir.eq.1) dalphadj = dalphadj*drds(x1)
      ...
      if (default_beta^C.or.x1.gt.rmax) then
        ...
        if (.not.present(dbetaidj)) then 
          ...
          if (present(jdir)) then
            dbetaidj = centered_difference(x, beta^C_, jdir)
            if (present(dbetaidj_iszero)) dbetaidj_iszero = .false.
          ! If using logarithmic radial coordinate, transform metric element
          ! Second term works because derivateve of exp(s) is exp(s)
            if (islog.and.jdir.eq.1) then 
              call interpolate_metric(x,beta^C_,beta)
              dbetaidj=dbetaidj*drds(x1) + beta*drds(x1)
            end if
          end if
        end if
  ...
  subroutine get_g_component(iin,jin,x^D,g,iszero,dgdk_iszero,dgdk,kdir)
    ...
    if (.not.present(kdir)) then
      call interpolate_metric(x,component_,g)
    else
      if (present(dgdk)) then
        dgdk = centered_difference(x, component_, kdir)
      end if
      if (present(dgdk_iszero)) dgdk_iszero = .false.
    end if

    if (islog) call mpistop("for g component the logarithmic coordinate transform is currently not implemented")
...
!           else
!             dgdk = centered_difference(x, g11_, kdir)
!             if (islog .and. jdir .eq. 1) then
!               call interpolate_metric(x,g11_,g)
!               dgdk=(drds(x1)**2)*(dgdk*drds(x1) + 2.0d0*g)
!               !g=g*drds(x1)**2  !FIXME I don't think g is ever required from this part
!             end if
!           end if
!         end if
!       case(2)
...
!           else
!             dgdk = centered_difference(x, g12_, kdir)
...
!     end select
!    if (mype == 0) then
!       if(g .ne. g) write(*,*) "and yes, it's also a NaN g sometimes"  ! MPI always appeal
!    endif
