...
!================================================================================
  ...
  !=============================================================================
  ! Metric components for the metric of an axisymmetric rotating neutron star  -coord=rns
  ...
  character*20, parameter              :: coord="rns"
...
contains
  ...
  subroutine get_alpha(x^D,alpha,iszero,dalphadj_iszero,dalphadj,jdir)
    ...
    ! .. local ..
    integer, parameter                               :: SDIV = 301
    integer, parameter                               :: MDIV = 151                                  
    !-----------------------------------------------------------------------------
    ...
    if(present(jdir)) then
		...
		if(jdir .eq. 1 .or. jdir .eq. 2) dalphadj_iszero = .false.
                if(jdir .eq. 3) dalphadj_iszero = .true.
	end if
     end if	
   	...
   	if(present(jdir)) then
 		call import_alpha_rns(x^D, nu_out, alpha, SDIV, MDIV, dalphadj, jdir)
		!if(jdir .eq. 3) dalphadj_iszero = .true.
	
	!if(jdir == 2) then
	!print*,x^D,  alpha, dalphadj
	!end if
	
	
   	else 
   		call import_alpha_rns(x^D, nu_out, alpha, SDIV, MDIV)
  	...
  	print*, "star is not yet initialized"
  	end if 
  ...
  subroutine get_beta(idir,x^D,beta,iszero,dbetaidj_iszero,dbetaidj,jdir)
    ...
    if(idir .eq. 1 .or. idir .eq. 2) dbetaidj_iszero =.true.
    	if(jdir .eq. 3 ) then
    ...
    if(is_initialized .eqv. .true.) then    
    	if(present(jdir)) then
         	call import_beta_rns(idir, x^D, nu_out, B_out, omega_out, beta, SDIV, MDIV, dbetaidj, jdir )
                !if(jdir == 2) print*, x^D,  beta, dbetaidj
    	else
         	call import_beta_rns(idir, x^D, nu_out, B_out, omega_out, beta, SDIV, MDIV) 
    	end if
    else
	print*, "Star not initialized yet, beta cannot be computed!"
    end if
  ...
  subroutine get_g_component(iin,jin,x^D,g,iszero,dgdk_iszero,dgdk,kdir)
    ...
    if(present(iszero)) then
    	if(iin .ne. jin) iszero = .true.
    end if

    if(present(dgdk_iszero)) then
    	if(iin .ne. jin) dgdk_iszero = .true.
	if(kdir .eq. 3) dgdk_iszero = .true.
    end if
    ...
    if (i==j) then
       ...
       if (i .eq. 1) then ! r direction
          call import_metric_component(i,j,x^D, g,nu_out,B_out,alpha_out,omega_out,SDIV, MDIV)
          if (present(kdir)) then
             if (kdir .eq.1) then
                if (present(dgdk)) &
                     call import_metric_component(i,j,x^D, g,nu_out,B_out,alpha_out,omega_out,SDIV, MDIV,dgdk,kdir)
	   ...
	   !print*, "metric component is zero."
