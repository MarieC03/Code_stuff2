...
character*20, parameter     :: coord="num"
logical, save               :: init_from_g4=.false.
integer, parameter          :: ncoordpar=1
double precision, save      :: coordpar(ncoordpar)

logical, save               :: islog=.false. ! True if spherical r coordinate is logarithmic
...
!=============================================================================
  subroutine get_alpha(x^D,alpha,iszero,dalphadj_iszero,dalphadj,jdir)
    ...
    ! Schwarzschild lapse
      if (islog) then
        alpha=sqrt(one - 2.0d0*eqpar(m_)/r(x1))
      else
        alpha=sqrt(one - 2.0d0*eqpar(m_)/x1)
      end if
...
!      dalphadj_iszero = .true.
      ...
      if (present(dalphadj)) then
        if (jdir.ne.1) then
          ...
          if (islog) then
            dalphadj = drds(x1)*eqpar(m_)/(sqrt(one - 2.0d0*eqpar(m_)/r(x1))*r(x1)**2)
          else
            dalphadj = one*eqpar(m_)/(sqrt(one - 2.0d0*eqpar(m_)/x1)*x1**2)
          end if
        end if
      end if
    else
...
{       case(^C)
          ! Radial derivative:
          if (present(dalphadj)) then
            ! call interpolate_metric(x,d^Calpha_,dalphadj)
             dalphadj=d^Calpha
          end if
          if (present(dalphadj_iszero)) then
