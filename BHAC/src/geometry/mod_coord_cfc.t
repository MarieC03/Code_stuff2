...
!=============================================================================
  subroutine init_coord
    ! cfc coordinate decided in amrvacusr.t
    ...
    else if (coordinate == cylindrical) then
    coord = 'cfc_cylind'
    if (^Z .ne. 2) call mpistop('Use cfc cylindrical, ^Z must be = 2')
    !only need cfc_cylind is okay, no need cylind
    get_gammainv_component_analytic => get_gammainv_component_analytic_cfc_cylind
    get_gammainv_component => get_gammainv_component_analytic
    call switch_coord_to_cfc_cylind
   
    endif

  end subroutine init_coord

  subroutine switch_coord_to_cfc_cylind
      get_sqrtgamma_analytic  => get_sqrtgamma_analytic_cfc_cylind
      get_alpha               => get_alpha_cfc_cylind
      get_beta                => get_beta_cfc_cylind
      get_g_component         => get_g_component_cfc_cylind
  end subroutine switch_coord_to_cfc_cylind

  subroutine switch_coord_to_cylind
      get_sqrtgamma_analytic  => get_sqrtgamma_analytic_cylind
      get_alpha               => get_alpha_cylind
      get_beta                => get_beta_cylind
      get_g_component         => get_g_component_cylind
  end subroutine switch_coord_to_cylind
  ...
  !--------CFC  cylindrical coordinate---------!
  subroutine get_sqrtgamma_analytic_cfc_cylind(x^D,sqrtgamma,is_analytic,w_pt)
    ...
    if(present(w_pt)) then
       ...
       w_pt_local(psi_metric_) = 1.0d0  ! hot fix for sth what without input of w_Pt
    endif

    if (w_pt_local(psi_metric_) .lt. 1.0d0 &
            .or. w_pt_local(psi_metric_) .ge. 3.0d0) then
       !write(*,*) w_pt_local(psi_metric_), 'psi_metric not filled in sqrtgamma' 
       !call mpistop ('psi met not filled well in sqrtgamma cfc cylind or unphysical values')
       !w_pt_local(psi_metric_) = 1.0d0
    endif


    sqrtgamma = x1 * w_pt_local(psi_metric_) **6.0d0

!    if (sqrtgamma .le. 0.0d0) then
!        write(*,*) 'sqrtgamma, x1  , x^Z'
!        write(*,*) sqrtgamma, x1  {^IFZIN, x^Z }
!        stop 'sqrtgamma in cfc cylind .le. 0.0d0'
!    endif

  end subroutine get_sqrtgamma_analytic_cfc_cylind

!=============================================================================
  subroutine get_alpha_cfc_cylind(x^D,alpha,iszero,dalphadj_iszero,dalphadj,jdir,w_pt)
    ...
    if(present(w_pt)) then
           ...
           w_pt_local(alp_metric_) = 1.0d0  ! hot fix for sth what without input of w_Pt
    endif

    if (w_pt_local(alp_metric_) .gt. 1.0d0 &
            .or. w_pt_local(alp_metric_) .le. 0.01d0) then   
       !write(*,*) w_pt_local(alp_metric_), 'alp not well filled in get alp'
       !call mpistop ('alp met not filled well in get_alp cfc cylind or unphysical values')
    ...
    if(alpha .ne. alpha) then
         alpha = 1.0d0
           ...
           !write(*,*) alpha, w_pt_local(alp_metric_)
         !call mpistop("and yes, it's also a NaN alpha sometimes in cfc cylind")
    endif

  end subroutine get_alpha_cfc_cylind
  !=============================================================================
  ! beta(1,2,3) --> beta(r,z,theta)
  subroutine get_beta_cfc_cylind(idir,x^D,beta,iszero,dbetaidj_iszero,dbetaidj,jdir,w_pt)
    ...
    if(present(w_pt)) then
        ...
        w_pt_local = 0.0d0 ! hot fix for sth what without input of w_Pt
    endif 
    ...
    if(beta .ne. beta) then
         beta = 0.0d0
           ...
           !write(*,*) beta, w_pt_local(beta_metric1_),&
           !            w_pt_local(beta_metric2_), w_pt_local(beta_metric3_)
         !stop  "and yes, it's also a NaN beta sometimes in cfc cylind"
    endif


  end subroutine get_beta_cfc_cylind


  subroutine get_g_component_cfc_cylind(iin,jin,x^D,g,iszero,dgdk_iszero,dgdk,kdir,w_pt)
    ...
    if (w_pt_local(psi_metric_) .lt. 1.0d0 &
       ...
       !call mpistop ('psi met not filled well in get g cfc cylind or unphysical values')
       !w_pt_local(psi_metric_) = 1.0d0
    endif
    ...
    if (i==j) then
       ...
       else if (i .eq. ^PHI) then ! phi direction (PHI = 3)

          g = x1**2.0d0 * w_pt_local(psi_metric_) ** 4.0d0

          if (present(kdir)) then
                if (present(dgdk_iszero)) dgdk_iszero = .true.
          end if

       else if (i .eq. ^Z) then ! z direction (Z = 2)
          g = w_pt_local(psi_metric_) ** 4.0d0
    ...
    if(g .ne. g) then 
         g = 1.0d0
           ...
           !write(*,*) g, w_pt_local(psi_metric_)
         !call mpistop("and yes, it's also a NaN g sometimes in cfc cylind")
    endif

  end subroutine get_g_component_cfc_cylind


  subroutine get_gammainv_component_analytic_cfc_cylind(iin,jin,x^D,ginv,iszero,dginvdk_iszero,dginvdk,kdir,w_pt)
    ...
    if (w_pt_local(psi_metric_) .lt. 1.0d0 &
       ...
       !call mpistop('psi met not filled well in get gammainv cfc cylind or unphysical values')
       !w_pt_local(psi_metric_) = 1.0d0
    endif
    ...
    if(ginv .ne. ginv) then 
         ...
         !  write(*,*) ginv, w_pt_local(psi_metric_)
         !stop  "and yes, it's also a NaN ginv sometimes in cfc cylind" 
    endif


   call mpistop('passed gammainv_componet analytic')

  end subroutine get_gammainv_component_analytic_cfc_cylind

!=============================================================================
!  Below are flat metric
!=============================================================================

  subroutine get_sqrtgamma_analytic_cylind(x^D,sqrtgamma,is_analytic,w_pt)
  ...
  end subroutine get_sqrtgamma_analytic_cylind

!=============================================================================
  subroutine get_alpha_cylind(x^D,alpha,iszero,dalphadj_iszero,dalphadj,jdir,w_pt)
    ...
    if (present(dalphadj)) then
         call mpistop('cfc not calucating metric dervatives here! here is get_alpha_cylind')
    endif

    alpha = 1.0d0

  end subroutine get_alpha_cylind
  !=============================================================================
  subroutine get_beta_cylind(idir,x^D,beta,iszero,dbetaidj_iszero,dbetaidj,jdir,w_pt)
    ...
    ! \partial_j \beta^i
    if (present(dbetaidj)) then
         call mpistop('cfc not calucating metric dervatives here! here is get_beta_cylind')
    endif
    if (present(dbetaidj_iszero)) &
         dbetaidj_iszero = .true. !.false.

  end subroutine get_beta_cylind
  !=============================================================================
  subroutine get_g_component_cylind(iin,jin,x^D,g,iszero,dgdk_iszero,dgdk,kdir,w_pt)
    ...
    if (i==j) then
          ...
          if (present(kdir)) then
             if (kdir .eq.1) then
                ...
                if (present(dgdk)) then
                    call mpistop('cfc not calucating metric dervatives here! here is get_g_com_cylind')
                endif
             else
                if (present(dgdk_iszero)) dgdk_iszero = .true. !.false.
                if (present(dgdk)) then
       ...
       else if (i .eq. ^Z) then ! z direction
          g = 1.0d0
  ...
  end subroutine get_g_component_cylind



  !--------CFC  spherical coordinate---------!
  subroutine get_sqrtgamma_analytic_cfc_sp(x^D,sqrtgamma,is_analytic,w_pt)
    ...
    if (w_pt_local(psi_metric_) .lt. 1.0d0 &
            .or. w_pt_local(psi_metric_) .ge. 3.0d0) then
       !write(*,*) w_pt_local(psi_metric_), 'psi_metric not filled in sqrtgamma' 
       !call mpistop ('psi met not filled well in sqrtgamma cfc sp or unphysical values')
       !w_pt_local(psi_metric_) = 1.0d0
    endif
...
!        write(*,*) 'sqrtgamma, x1  , x^Z'
!        write(*,*) sqrtgamma, x1  {^IFZIN, x^Z }
!        stop 'sqrtgamma in cfc sp .le. 0.0d0'
!    endif
...
!=============================================================================
  subroutine get_alpha_cfc_sp(x^D,alpha,iszero,dalphadj_iszero,dalphadj,jdir,w_pt)
    ...
    if (w_pt_local(alp_metric_) .gt. 1.0d0 &
            .or. w_pt_local(alp_metric_) .le. 0.01d0) then   
       !write(*,*) w_pt_local(alp_metric_), 'alp not well filled in get alp'
       !call mpistop ('alp met not filled well in get_alp cfc sp or unphysical values')
       !w_pt_local(alp_metric_) = 1.0d0
    endif
    ...
    if(alpha .ne. alpha) then
         ...
         !  write(*,*) 'alpha, w_pt_local(alp_metric_)'
         !  write(*,*) alpha, w_pt_local(alp_metric_)
         !call mpistop("and yes, it's also a NaN alpha sometimes in cfc sp")
    endif
  ...
  subroutine get_beta_cfc_sp(idir,x^D,beta,iszero,dbetaidj_iszero,dbetaidj,jdir,w_pt)
    ...
    if(beta .ne. beta) then
         ...
         !  write(*,*) beta, w_pt_local(beta_metric1_),&
         !              w_pt_local(beta_metric2_), w_pt_local(beta_metric3_)
         !stop  "and yes, it's also a NaN beta sometimes in cfc sp"
    endif
  ...
  subroutine get_g_component_cfc_sp(iin,jin,x^D,g,iszero,dgdk_iszero,dgdk,kdir,w_pt)
    ...
    if (w_pt_local(psi_metric_) .lt. 1.0d0 &
            .or. w_pt_local(psi_metric_) .ge. 3.0d0) then
       !write(*,*) w_pt_local(psi_metric_), 'psi_metric not well filled in g_com'
       !call mpistop ('psi met not filled well in get g cfc sp or unphysical values')
       !w_pt_local(psi_metric_) = 1.0d0
    endif
    ...
    if(g .ne. g) then 
         ...
         !  write(*,*) 'g, w_pt_local(psi_metric_)'
         !  write(*,*) g, w_pt_local(psi_metric_)
         !call mpistop("and yes, it's also a NaN g sometimes in cfc sp")
    endif
...
!  end subroutine get_g_component_cfc_sp
  ...
  subroutine get_gammainv_component_analytic_cfc_sp(iin,jin,x^D,ginv,iszero,dginvdk_iszero,dginvdk,kdir,w_pt)
    ...
    if (w_pt_local(psi_metric_) .lt. 1.0d0 &
            .or. w_pt_local(psi_metric_) .ge. 3.0d0) then
       !write(*,*) w_pt_local(psi_metric_), 'psi_metric not well filled in get gammainv'
       !call mpistop('psi met not filled well in get gammainv cfc sp or unphysical values')
       !w_pt_local(psi_metric_) = 1.0d0
    endif
    ...
    if(ginv .ne. ginv) then 
         ...
         !  write(*,*) 'ginv, w_pt_local(psi_metric_)'
         !  write(*,*) ginv, w_pt_local(psi_metric_)
         !stop  "and yes, it's also a NaN ginv sometimes in cfc sp" 
    endif
...
!=============================================================================
  ...
  subroutine get_sqrtgamma_analytic_cfc_cart(x^D,sqrtgamma,is_analytic,w_pt)
    ...
    if (w_pt_local(psi_metric_) .lt. 1.0d0 &
            .or. w_pt_local(psi_metric_) .ge. 3.0d0) then
       !write(*,*) w_pt_local(psi_metric_), 'psi_metric not filled in sqrtgamma'
       !call mpistop('psi met not filled well in get sqrtgamma cfc cart or unphysical values')
       !w_pt_local(psi_metric_) = 1.0d0
    endif
...
!=============================================================================
  subroutine get_alpha_cfc_cart(x^D,alpha,iszero,dalphadj_iszero,dalphadj,jdir,w_pt)
    ...
    if (w_pt_local(alp_metric_) .gt. 1.0d0 &
            .or. w_pt_local(alp_metric_) .le. 0.01d0) then   ! < 0.4 ?
       !write(*,*) w_pt_local(alp_metric_), 'alp not well filled in get alp'
       !call mpistop('alp met not filled well in get alp in cfc cart or unphysical values')
       !w_pt_local(alp_metric_) = 1.0d0
    endif
    ...
    if(alpha .ne. alpha) then
        ...
        !   write(*,*) 'alpha, w_pt_local(alp_metric_)'
        !   write(*,*) alpha, w_pt_local(alp_metric_)
        !   write(*,*) 'at location', x^D
        ! call mpistop("and yes, it's also a NaN alpha sometimes in cfc cart")
    endif
  ...
  subroutine get_beta_cfc_cart(idir,x^D,beta,iszero,dbetaidj_iszero,dbetaidj,jdir,w_pt)
    ...
    if(beta .ne. beta) then
         ...
         !  write(*,*) beta, w_pt_local(beta_metric1_),&
         !              w_pt_local(beta_metric2_), w_pt_local(beta_metric3_)
         !stop  "and yes, it's also a NaN beta sometimes in cfc cart"
    endif
  ...
  subroutine get_g_component_cfc_cart(iin,jin,x^D,g,iszero,dgdk_iszero,dgdk,kdir,w_pt)
    ...
    if (w_pt_local(psi_metric_) .lt. 1.0d0 &
            .or. w_pt_local(psi_metric_) .ge. 3.0d0) then
       !write(*,*) w_pt_local(psi_metric_), 'psi_metric not well filled in g_com'
       !call mpistop('psi met not filled well in get g cfc cart or unphysical values')
       !w_pt_local(psi_metric_) = 1.0d0
    endif
    ...
    if(g .ne. g) then
         ...
         !  write(*,*) 'g, w_pt_local(psi_metric_)'
         !  write(*,*) g, w_pt_local(psi_metric_)
         !call mpistop ("and yes, it's also a NaN g sometimes in cfc cart")
    endif
  ...
  subroutine get_gammainv_component_analytic_cfc_cart(iin,jin,x^D,ginv,iszero,dginvdk_iszero,dginvdk,kdir,w_pt)
    ...
    if (w_pt_local(psi_metric_) .lt. 1.0d0 &
            .or. w_pt_local(psi_metric_) .ge. 3.0d0) then
       !write(*,*) w_pt_local(psi_metric_), 'psi_metric not well filled in get gammainv'
       !call mpistop('psi met not filled well in gammainv cfc cart or unphysical values')
       !w_pt_local(psi_metric_) = 1.0d0
    endif
    ...
    if(ginv .ne. ginv) then
         ...
         !  write(*,*) 'ginv, w_pt_local(psi_metric_)'
         !  write(*,*) ginv, w_pt_local(psi_metric_)
         !stop  "and yes, it's also a NaN ginv sometimes in cfc cart"
    endif
...
!=============================================================================
  ...
  ! not that useful coordinate trans
