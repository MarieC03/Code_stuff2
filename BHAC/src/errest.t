...
subroutine lohner_grid(igrid)
  ...
  do iflag=1,nw+1
     ...
     if(wflags(iflag)==0.d0) cycle
     !if(w_refine_weight(iflag)==0.d0) cycle
     numerator=zero

     if (iflag > nw) then
       call specialvarforerrest(ixG^LL,ixG^LL,iflag,pw(igrid)%w,tmp1)
     end if
     ...
     error=error+wflags(iflag)*dsqrt(numerator/max(denominator,epsilon))
  end do
  ...
  {do ix^DB=ixMlo^DB,ixMhi^DB\}
    {#IFDEF SPECIALTOLERANCE
     ...
     else if (error(ix^D) <= tolratio(level)*threshold) then
...
! only check w(ixG^T, 1)   the first one variable..--> D = rhoW
!subroutine lohner_grid(igrid)
!use mod_forest, only: coarsen, refine
!include 'amrvacdef.f'
!
...
!
!write(*,*) flags(nflag_),'finally arrive inside flags(nflag)'
!write(*,*) iflag, 'iflag'
!
!   numerator=zero
...
!$OMP PARALLEL DO PRIVATE(igrid)
do iigrid=1,igridstail; igrid=igrids(iigrid);
   call createCoarse_grid(igrid,psCoarse(igrid),pxCoarse(igrid),ixG^L,psold(igrid),px(igrid)%x)
end do
!$OMP END PARALLEL DO

end subroutine createCoarse
!=============================================================================
subroutine createCoarse_grid(igrid,sCo,pxCo,ixCoG^L,sold,xold)

include 'amrvacdef.f'

integer, intent(in) :: igrid, ixCoG^L
type(state)         :: sCo, sold
double precision    :: xold(ixG^T,1:ndim)
type(xalloc)        :: pxCo
! .. local ..
integer             :: ixCoM^L
!-----------------------------------------------------------------------------
...
call coarsen_grid(sold,xold,ixG^LL,ixM^LL,sCo,pxCo%x,ixCoG^L,ixCoM^L, &
...
double precision :: fC(psCo%w%ixG^S,1:nwflux,1:ndim)
integer :: level
!-----------------------------------------------------------------------------
...
level=node(plevel_,igrid)
call advect1_grid(typelow1(level),qdt,psCo%w%ixG^L,1,ndim,t,sCo1,t, &
                  psCo,sCo1,fC,dx^D,pxCoarse(igrid)%x,sm1)
...
if (.not.skipfinestep) then
   ...
   qdt=dt_grid(igrid)
   call advect1_grid(typelow1(level),qdt,s%w%ixG^L,1,ndim,t+qdt,s, &
                     t+qdt,sFi,s,fC,dx^D,px(igrid)%x,sm1)
end if

ixCoGmin^D=1;
ixCoGmax^D=ixGhi^D/2+dixB;

call flagbadpoints(sFi%w%w,pwCoarse(igrid)%w,ixCoG^L,igrid,level)
