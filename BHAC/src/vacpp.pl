...
vacpp.pl -d=NdimNdir -p=PHYSICS [-phi=Idirphi] [-z=Idirz] [-eos=typeeos] [-nf=Nfluids] [-nspecies=Nspecies] [-ndust=Ndust] [-cp=mpienv]\
    file.t > file.f
...
$nf=0 unless $nf;
$nd=0 unless $nd;
$nspecies=0 unless $nspecies; 
$cp=openmpi unless $cp;
$eos=default unless $eos;
$coord=cart unless $coord;
...
sub processfile {
   ...
   while (<$input>) {
      ...
      if (/INCLUDE:(.*)/) {&processfile("$BHAC_DIR/src/$1",$input);next}
...
sub definepatterns{
   ...
   &patdef('NS', 1      ,$nspecies              );
   &patdef('ND'	,1	,$ndim			);
   &patdef('NC'	,1	,$ndir			);
   &patdef('NFL',1	,$nf			);
   &patdef('PHI',1	,$phi			);
   &patdef('Z'	,1	,$z			);
   &patdef('COORD'  ,1	,$coord    		);   
...
#  Dust specific patterns:
   ...
   &patdef('KSP', $nspecies ,1,2,3,4,5,6);
   &patdef('KSP&', $nspecies                    );
   ...
   &patdef('IFPHI'      ,$phi>0			);
   &patdef('IFNOPHI'    ,$phi<=0		);
   &patdef('IFZ'        ,$z>0			);
   &patdef('IFNOZ'      ,$z<=0			);
   &patdef('IFZIN'     ,$z<=$ndim && $z>0       );
   &patdef('IFPHIIN'   ,$phi<=$ndim   && $phi>0 );
   &patdef('IFZOUT'    ,$z>$ndim || $z<=0       );
   &patdef('IFPHIOUT'  ,$phi>$ndim || $phi<=0   );

# Select the current coordinates:
   &patdef(join('','IFCOORD',uc "$coord") , 1 eq 1   );
# Select the current physics:
   &patdef(join('','IFPHYS',uc "$p") , 1 eq 1   );
...
sub block{
   ...
   while($rightpos>=length($_) && $nline<3000 && ($line=<$input>)){
       $_.=&unquote($line); $nline++;
       $rightpos=&rightmatch($rightpos-1,$ope,$clo,$clo);
       #TST print "Line:$_ Rightpos:$rightpos\n";
   }
   die"$ope without matching $clo within 3000 lines" if $rightpos>=length($_);
