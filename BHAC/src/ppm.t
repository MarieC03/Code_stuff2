...
!    ! author: zakaria.meliani@wis.kuleuven.be
  ...
  subroutine PPMflattening(ixI^L,hxC^L,idims,w,wL,wR,flat_cd,flat_sh)
    ...
    use mod_imhd_intermediate, only:imhd_ppm_flatsh,imhd_ppm_flatcd
    include 'amrvacdef.f'

    integer, intent(in)             :: ixI^L, hxC^L, idims
    double precision, intent(in)    :: w(ixI^S,1:nprim)
    double precision, intent(inout) :: wR(ixI^S,1:nprim),wL(ixI^S,1:nprim)
    logical, intent(in)             :: flat_cd, flat_sh

    double precision, dimension(ixI^S,1:nprim)  :: dwC,d2wC,ldw
    double precision, dimension(ixI^S,1:nprim)  :: wMin,wMax
    ...
    dwC(lxC^S,1:nprim)=w(lxR^S,1:nprim)-w(lxC^S,1:nprim)
    ! Eq. 64,  Miller and Colella 2002, JCP 183, 26
    d2wC(kxC^S,1:nprim)=half*(w(kxR^S,1:nprim)-w(kxL^S,1:nprim))
    where(dwC(kxC^S,1:nprim)*dwC(kxL^S,1:nprim)>zero)
       ! Store the sign of dwC in wMin
       wMin(kxC^S,1:nprim)= sign(one,d2wC(kxC^S,1:nprim))
       ! Eq. 65,  Miller and Colella 2002, JCP 183, 26
       ldw(kxC^S,1:nprim)= wMin(kxC^S,1:nprim)*min(dabs(d2wC(kxC^S,1:nprim)),&
            2.0d0*dabs(dwC(kxL^S,1:nprim)),&
            2.0d0*dabs(dwC(kxC^S,1:nprim)))
    elsewhere
       ldw(kxC^S,1:nprim)=zero
    endwhere

    ! flattening at the contact discontinuities
    if(flat_cd)then
       ! Note that aa here is drho, ab is dpress
       call imhd_ppm_flatcd(ixI^L,kxC^L,kxL^L,kxR^L,w,d2wC,aa,ab)
    ...
    if(flat_sh)then
       ...
       do idimss=1,ndim
          ...
          call imhd_ppm_flatsh(ixI^L,kxC^L,kxLL^L,kxL^L,kxR^L,kxRR^L,idimss,w,aa,ab,dv)
