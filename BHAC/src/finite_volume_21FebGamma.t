...
{#IFDEF M1
   ...
   {^KSP&
     ...
     w(ixI^S,nrad^KSP_) = w(ixI^S,nrad^KSP_) / metricM1%sqrtg(ixI^S)
     {^C& w(ixI^S,frad^KSP^C_) = w(ixI^S,frad^KSP^C_) / metricM1%sqrtg(ixI^S) \}
     call m1_update_closure(metricM1, w, x, ixI^L, ixI^L,^KSP,.true.,Gamma)
     !w(ixI^S,nrad^KSP_) = max(w(ixI^S,nrad^KSP_)/Gamma(ixI^S), m1_E_atmo) /(w(ixI^S,erad^KSP_)) 
     w(ixI^S,nrad^KSP_) = w(ixI^S,nrad^KSP_)/Gamma(ixI^S) /(w(ixI^S,erad^KSP_)) 
     ...
     wLp(ixL^S,nrad^KSP_) = wLp(ixL^S,nrad^KSP_) / metricM1_L%sqrtg(ixL^S)
     call m1_update_closure(metricM1_L, w, x, ixI^L, ixL^L,^KSP,.true.,Gamma_L)
     !wLp(ixL^S,nrad^KSP_) = max(wLp(ixL^S,nrad^KSP_)/Gamma_L(ixL^S),m1_E_atmo) /(wLp(ixL^S,erad^KSP_) ) 
     wLp(ixL^S,nrad^KSP_) = wLp(ixL^S,nrad^KSP_)/Gamma_L(ixL^S) /(wLp(ixL^S,erad^KSP_) ) 
     ...
     wRp(ixR^S,nrad^KSP_) = wRp(ixR^S,nrad^KSP_) / metricM1_R%sqrtg(ixR^S)
     call m1_update_closure(metricM1_R, w, x, ixI^L, ixR^L,^KSP,.true.,Gamma_R)
     !wRp(ixR^S,nrad^KSP_) = max(wRp(ixR^S,nrad^KSP_)/Gamma_R(ixR^S), m1_E_atmo) /(wRp(ixR^S,erad^KSP_)) 
     wRp(ixR^S,nrad^KSP_) = wRp(ixR^S,nrad^KSP_)/Gamma_R(ixR^S)/(wRp(ixR^S,erad^KSP_)) 
...
{#IFDEF M1
    {^KSP&
      ...
      call m1_update_closure(metricM1, w, x, ixI^L, ixI^L,^KSP,.true.,Gamma)
      call m1_update_closure(metricM1_L, wLp, x, ixI^L, ixL^L,^KSP,.true.,Gamma_L)
      call m1_update_closure(metricM1_R, wRp, x, ixI^L, ixR^L,^KSP,.true.,Gamma_R)

       !w(ixI^S,erad^KSP_) = max(w(ixI^S,erad^KSP_),m1_E_atmo)
       !w(ixI^S,nrad^KSP_) = max(w(ixI^S,nrad^KSP_),m1_E_atmo) * Gamma(ixI^S) * metricM1%sqrtg(ixI^S) * (w(ixI^S,erad^KSP_)) !KEN KEN removed M1Mini. No safeguard is needed for mult.
       w(ixI^S,nrad^KSP_) = w(ixI^S,nrad^KSP_) * Gamma(ixI^S) * metricM1%sqrtg(ixI^S) * (w(ixI^S,erad^KSP_)) !KEN KEN removed M1Mini. No safeguard is needed for mult.
       {^C& w(ixI^S,frad^KSP^C_) = w(ixI^S,frad^KSP^C_) * metricM1%sqrtg(ixI^S) * (w(ixI^S,erad^KSP_)) \}
       w(ixI^S,erad^KSP_) = (w(ixI^S,erad^KSP_)) * metricM1%sqrtg(ixI^S)

     ! wLp(ixL^S,erad^KSP_) = max(wLp(ixL^S,erad^KSP_),m1_E_atmo)
      !wLp(ixL^S,nrad^KSP_) = max(wLp(ixL^S,nrad^KSP_),m1_E_atmo) * Gamma_L(ixL^S) * metricM1_L%sqrtg(ixL^S) * (wLp(ixL^S,erad^KSP_))
      wLp(ixL^S,nrad^KSP_) = wLp(ixL^S,nrad^KSP_) * Gamma_L(ixL^S) * metricM1_L%sqrtg(ixL^S) * (wLp(ixL^S,erad^KSP_))
      {^C& wLp(ixL^S,frad^KSP^C_) = wLp(ixL^S,frad^KSP^C_) * metricM1_L%sqrtg(ixL^S) * (wLp(ixL^S,erad^KSP_)) \}
      wLp(ixL^S,erad^KSP_) = (wLp(ixL^S,erad^KSP_)) * metricM1_L%sqrtg(ixL^S)
 
      !wRp(ixR^S,erad^KSP_) = max(wRp(ixR^S,erad^KSP_),m1_E_atmo)
      !wRp(ixR^S,nrad^KSP_) = max(wRp(ixR^S,nrad^KSP_),m1_E_atmo) * Gamma_R(ixR^S) * metricM1_R%sqrtg(ixR^S) * (wRp(ixR^S,erad^KSP_))
      wRp(ixR^S,nrad^KSP_) = wRp(ixR^S,nrad^KSP_) * Gamma_R(ixR^S) * metricM1_R%sqrtg(ixR^S) * (wRp(ixR^S,erad^KSP_))
      {^C& wRp(ixR^S,frad^KSP^C_) = wRp(ixR^S,frad^KSP^C_) * metricM1_R%sqrtg(ixR^S) * (wRp(ixR^S,erad^KSP_)) \}
