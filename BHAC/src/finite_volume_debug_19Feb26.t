...
{#IFDEF M1
   ...
   {^KSP&
     ...
     !! in case fluxes overshoot....
     fmag(ixI^S) = dsqrt( {^C& w(ixI^S,frad^KSP^C_)**2 +} + M1_mini ) ! actually need Fup Flow, maybe no M1_mini
     scaleF(ixI^S) = 1.0d0  
     where (Fmag(ixI^S) > w(ixI^S,erad^KSP_)**2)
       scaleF(ixI^S) = dsqrt(w(ixI^S,erad^KSP_)**2/fmag(ixI^S)) * fmaxfact
     end where
     {^C& w(ixI^S,frad^KSP^C_) = w(ixI^S,frad^KSP^C_) * scaleF(ixI^S) \}
     !-------
     fmag(ixL^S) = dsqrt( {^C& wLp(ixL^S,frad^KSP^C_)**2 +} + M1_mini ) ! actually need Fup Flow, maybe no M1_mini
     scaleF(ixL^S) = 1.0d0  
     where (Fmag(ixL^S) > wLp(ixL^S,erad^KSP_)**2)
       scaleF(ixL^S) = dsqrt(wRp(ixL^S,erad^KSP_)**2/fmag(ixL^S)) * fmaxfact
     end where
     {^C& wLp(ixL^S,frad^KSP^C_) = wLp(ixL^S,frad^KSP^C_) * scaleF(ixL^S) \}
     !-------
     fmag(ixR^S) = dsqrt( {^C& wRp(ixR^S,frad^KSP^C_)**2 +} + M1_mini ) ! actually need Fup Flow, maybe no M1_mini
     scaleF(ixR^S) = 1.0d0  
     where (Fmag(ixR^S) > wRp(ixR^S,erad^KSP_)**2)
       scaleF(ixR^S) = dsqrt(wRp(ixR^S,erad^KSP_)**2/fmag(ixR^S)) * fmaxfact
     end where
     {^C& wRp(ixR^S,frad^KSP^C_) = wRp(ixR^S,frad^KSP^C_) * scaleF(ixR^S) \}
   \}  
...
{#IFDEF M1
    {^KSP&
       w(ixI^S,erad^KSP_) = max(w(ixI^S,erad^KSP_),m1_E_atmo)
       w(ixI^S,nrad^KSP_) = w(ixI^S,nrad^KSP_) * metricM1%sqrtg(ixI^S) * (w(ixI^S,erad^KSP_)) !KEN KEN removed M1Mini. No safeguard is needed for mult.
       {^C& w(ixI^S,frad^KSP^C_) = w(ixI^S,frad^KSP^C_) * metricM1%sqrtg(ixI^S) * (w(ixI^S,erad^KSP_)) \}
       w(ixI^S,erad^KSP_) = (w(ixI^S,erad^KSP_)) * metricM1%sqrtg(ixI^S)

      wLp(ixL^S,erad^KSP_) = max(wLp(ixL^S,erad^KSP_),m1_E_atmo)
      wLp(ixL^S,nrad^KSP_) = wLp(ixL^S,nrad^KSP_) * metricM1_L%sqrtg(ixL^S) * (wLp(ixL^S,erad^KSP_))
      {^C& wLp(ixL^S,frad^KSP^C_) = wLp(ixL^S,frad^KSP^C_) * metricM1_L%sqrtg(ixL^S) * (wLp(ixL^S,erad^KSP_)) \}
      wLp(ixL^S,erad^KSP_) = (wLp(ixL^S,erad^KSP_)) * metricM1_L%sqrtg(ixL^S)
 
      wRp(ixL^S,erad^KSP_) = max(wRp(ixL^S,erad^KSP_),m1_E_atmo)
      wRp(ixR^S,nrad^KSP_) = wRp(ixR^S,nrad^KSP_) * metricM1_R%sqrtg(ixR^S) * (wRp(ixR^S,erad^KSP_))
      {^C& wRp(ixR^S,frad^KSP^C_) = wRp(ixR^S,frad^KSP^C_) * metricM1_R%sqrtg(ixR^S) * (wRp(ixR^S,erad^KSP_)) \}
...
contains 
   ...
   subroutine positivity_preserving_limiter()
     ...
     do iw = 1, ncons
        ...
        {#IFDEF M1
           ! Skip ALL radiation vars here; we treat them per species below.
