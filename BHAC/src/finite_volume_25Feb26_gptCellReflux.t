...
double precision, dimension(ixI^S)    :: Gamma, Gamma_L, Gamma_R
...
{#IFDEF M1
   ...
   {^KSP&
     ...
     Eden(ixI^S) = (w(ixI^S,erad^KSP_)) / metricM1%sqrtg(ixI^S)
     w(ixI^S,erad^KSP_) = Eden(ixI^S)
     ...
     EdenLp(ixL^S) = (wLp(ixL^S,erad^KSP_)) / metricM1_L%sqrtg(ixL^S)
     wLp(ixL^S,erad^KSP_) = EdenLp(ixL^S)
     ...
     EdenRp(ixR^S) = (wRp(ixR^S,erad^KSP_)) / metricM1_R%sqrtg(ixR^S)
     wRp(ixR^S,erad^KSP_) = EdenRp(ixR^S)
...
{#IFDEF M1
    {^KSP&
       ! Undo the reconstruction normalization consistently:
       !   Erad = Erad_hat * sqrt(g)
       !   Nrad = (N_over_E) * sqrt(g) * (Erad_hat + mini)
       !   Frad = (F_over_E) * sqrt(g) * (Erad_hat + mini)
       Eden(ixI^S) = w(ixI^S,erad^KSP_) + M1_mini
       w(ixI^S,nrad^KSP_) = w(ixI^S,nrad^KSP_) * metricM1%sqrtg(ixI^S) * Eden(ixI^S)
       {^C& w(ixI^S,frad^KSP^C_) = w(ixI^S,frad^KSP^C_) * metricM1%sqrtg(ixI^S) * Eden(ixI^S) \}
       w(ixI^S,erad^KSP_) = w(ixI^S,erad^KSP_) * metricM1%sqrtg(ixI^S)

      EdenLp(ixL^S) = wLp(ixL^S,erad^KSP_) + M1_mini
      wLp(ixL^S,nrad^KSP_) = wLp(ixL^S,nrad^KSP_) * metricM1_L%sqrtg(ixL^S) * EdenLp(ixL^S)
      {^C& wLp(ixL^S,frad^KSP^C_) = wLp(ixL^S,frad^KSP^C_) * metricM1_L%sqrtg(ixL^S) * EdenLp(ixL^S) \}
      wLp(ixL^S,erad^KSP_) = wLp(ixL^S,erad^KSP_) * metricM1_L%sqrtg(ixL^S)

      EdenRp(ixR^S) = wRp(ixR^S,erad^KSP_) + M1_mini
      wRp(ixR^S,nrad^KSP_) = wRp(ixR^S,nrad^KSP_) * metricM1_R%sqrtg(ixR^S) * EdenRp(ixR^S)
      {^C& wRp(ixR^S,frad^KSP^C_) = wRp(ixR^S,frad^KSP^C_) * metricM1_R%sqrtg(ixR^S) * EdenRp(ixR^S) \}
      wRp(ixR^S,erad^KSP_) = wRp(ixR^S,erad^KSP_) * metricM1_R%sqrtg(ixR^S)

      where ((wLp(ixL^S, erad^KSP_) < m1_E_atmo) .or. (wLp(ixL^S, nrad^KSP_) < m1_E_atmo))
        wLp(ixL^S, erad^KSP_) = m1_E_atmo
        wLp(ixL^S, nrad^KSP_) = m1_E_atmo
        {^C& wLp(ixL^S, frad^KSP^C_) = 0.0d0 \}
      end where

      where (wRp(ixR^S, erad^KSP_) < m1_E_atmo .or. wRp(ixR^S, nrad^KSP_) < m1_E_atmo)
        wRp(ixR^S, erad^KSP_) = m1_E_atmo
        wRp(ixR^S, nrad^KSP_) = m1_E_atmo
...
{#IFDEF M1
logical, dimension(ixI^S) :: need_fallback, need_iface
double precision, dimension(ixI^S) :: nrad_pred
integer :: ixCp^L
}

{#IFDEF M1
double precision, dimension(ixI^S,1:nm1rad_eas)            :: wradimpl
logical, parameter :: m1_fallback_flux = .true.
logical, parameter :: m1_debug_wavespeeds = .true.
integer,  parameter :: m1_debug_maxprint = 20
integer :: m1_debug_printcount
}
...
{#IFDEF M1
   if (m1_fallback_flux) then
      allocate(fC_low(ixI^S,1:ncons,1:ndim))
      fC_low=0.0d0
   end if
}
m1_debug_printcount = 0
end if
...
{#IFDEF STAGGERED
   ...
   ! --- Store a low-order TVDLF flux for M1 fallback (even when PP_limiter is off)
{#IFDEF M1
   if (allocated(fC_low) .and. .not.PP_limiter .and. m1_fallback_flux) then
      ! compute TVDLF flux into fC, store it, then overwrite fC with the chosen solver flux below
      call get_Riemann_flux_tvdlf()
      do iw=1,ncons
         fC_low(ixC^S,iw,idims)=fC(ixC^S,iw,idims)
      end do
   ...
   !do iw=1,nwflux
      if (slab) then
         ...
         select case (idims)
            ...
            {#IFDEF M1
            if (allocated(fC_low) .and. m1_fallback_flux) fC_low(ixC^S,iw,^D) = mygeo%surfaceC^D(ixC^S)*fC_low(ixC^S,iw,^D)
            }
            elsewhere
               fC(ixC^S,iw,^D) = zero
            end where
...
{#IFDEF M1
   if (allocated(fC_low) .and. m1_fallback_flux) then
      ! If the tentative update would make nrad negative, locally fall back to TVDLF flux
      {^KSP&
         ! cell-wise check (using *current* wnew before applying this dimension's contribution)
         if (slab) then
            nrad_pred(ixO^S) = wnew(ixO^S,nrad^KSP_) + &
                 dxinv(idims)*( fC(ixO^S,nrad^KSP_,idims)-fC(hxO^S,nrad^KSP_,idims) )
            need_fallback(ixO^S) = (nrad_pred(ixO^S) < m1_E_atmo) .or. (nrad_pred(ixO^S) /= nrad_pred(ixO^S))
         else
            select case (idims)
            {case (^D)
               nrad_pred(ixO^S) = wnew(ixO^S,nrad^KSP_) + &
                    (-qdt)*( fC(ixO^S,nrad^KSP_,^D)-fC(hxO^S,nrad^KSP_,^D) )/mygeo%dvolume(ixO^S)
               need_fallback(ixO^S) = (nrad_pred(ixO^S) < m1_E_atmo) .or. (nrad_pred(ixO^S) /= nrad_pred(ixO^S))
            \}
            end select
         end if

         if (any(need_fallback(ixO^S))) then
{#IFDEF M1
if (m1_debug_wavespeeds .and. m1_debug_printcount < m1_debug_maxprint) then
   m1_debug_printcount = m1_debug_printcount + 1
   print *, "M1 cell-fallback triggered: idims=",idims," species base nrad=",nrad^KSP_," min(nrad_pred)=",minval(nrad_pred(ixO^S))
end if
}
            ! apply low-order flux on both interfaces of any flagged cell
            where (need_fallback(ixO^S))
               fC(ixO^S,nrad^KSP_,idims) = fC_low(ixO^S,nrad^KSP_,idims)
               fC(ixO^S,erad^KSP_,idims) = fC_low(ixO^S,erad^KSP_,idims)
               {^C& fC(ixO^S,frad^KSP^C_,idims) = fC_low(ixO^S,frad^KSP^C_,idims) \}
               fC(hxO^S,nrad^KSP_,idims) = fC_low(hxO^S,nrad^KSP_,idims)
               fC(hxO^S,erad^KSP_,idims) = fC_low(hxO^S,erad^KSP_,idims)
               {^C& fC(hxO^S,frad^KSP^C_,idims) = fC_low(hxO^S,frad^KSP^C_,idims) \}
            end where
         end if
      \} ! end species loop
   end if
...
{#IFDEF M1
   ! ------------------------------------------------------------------
   ! Emergency positivity fix (non-conservative) as a last-resort guard:
   ! If nrad still went negative after the conservative fallback, clamp it.
   ! This should normally NEVER trigger; if it does, enable m1_debug_wavespeeds.
   ! ------------------------------------------------------------------
   {^KSP&
      where (wnew(ixO^S,nrad^KSP_) < m1_E_atmo)
         wnew(ixO^S,nrad^KSP_) = m1_E_atmo
         wnew(ixO^S,erad^KSP_) = max(wnew(ixO^S,erad^KSP_), m1_E_atmo)
         {^C& wnew(ixO^S,frad^KSP^C_) = 0.0d0 \}
      end where
   \}
}
...
contains 
   ...
   subroutine get_Riemann_flux_tvdlf()
     ...
     do iw=1, ncons
       ...
       fdiss(ixC^S)=-tvdlfeps*max(dabs(cmaxC(ixC^S,iw)), dabs(cminC(ixC^S,iw)))*half*(wRC(ixC^S,iw)-wLC(ixC^S,iw))
   ...
   subroutine get_Riemann_flux_hll()
              integer :: ix^D
       double precision :: denom, cmin_loc, cmax_loc, alpha_loc
     ...
     do iw=1, ncons
       ...
       {do ix^DB=ixCmin^DB,ixCmax^DB\}
         if(cminC(ix^D,iw) >= 0.0d0) then
            ...
            ! Add hll dissipation to the flux (with wavespeed sanity + positivity-safe fallback)
cmax_loc = cmaxC(ix^D,iw)
cmin_loc = cminC(ix^D,iw)

! Enforce consistent HLL bounds (important if any wavespeed routine returns same-signed speeds)
if (cmax_loc < 0.0d0) cmax_loc = 0.0d0
if (cmin_loc > 0.0d0) cmin_loc = 0.0d0

denom = cmax_loc - cmin_loc

if (.not.(denom > 1.0d-14) .or. (denom /= denom)) then
   ! Degenerate / inconsistent wavespeeds -> use LLF/TVDLF (monotone) flux
   alpha_loc = max(dabs(cmax_loc), dabs(cmin_loc))
   fC(ix^D,iw,idims) = 0.5d0*(fLC(ix^D,iw) + fRC(ix^D,iw)) - 0.5d0*tvdlfeps*alpha_loc*(wRC(ix^D,iw)-wLC(ix^D,iw))
   {#IFDEF M1
   if (m1_debug_wavespeeds .and. m1_debug_printcount < m1_debug_maxprint) then
      m1_debug_printcount = m1_debug_printcount + 1
      print *, "M1/HLL wavespeed fix (degenerate): idims=",idims," iw=",iw," cmin=",cminC(ix^D,iw)," cmax=",cmaxC(ix^D,iw)
   end if
   }
else
   fC(ix^D,iw,idims) = (cmax_loc*fLC(ix^D,iw)-cmin_loc*fRC(ix^D,iw) &
        + tvdlfeps*cmin_loc*cmax_loc*(wRC(ix^D,iw)-wLC(ix^D,iw))) / denom
end if
   ...
   subroutine positivity_preserving_limiter_Ken()
     use mod_eos, only: small_rho
     implicit none
   ...
   if (slab) then
     ...
     call get_theta(ixI^L,ixC^L,idims,epsD,inv_lambda(ixI^S),sCT%w%w(ixI^S,nrad^KSP_),flow_dA,fhigh_dA,theta)
...
!
   ...
   ! where(A(ixC^S) > 1.0d0) A(ixC^S) = 1.0d0
    ...
    if(any(phi(ixC^S) .gt. 0.9)) then
      print *, "phi for negative nrad is!", maxval(phi(ixC^S))
    endif
   ...
   end subroutine positivity_preserving_limiter_Ken
