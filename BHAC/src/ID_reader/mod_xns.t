...
subroutine mod_XNS_read_profile(profile_path,xns_profile_type)
  ...
  integer :: Nth, Nr
  integer :: i,j
  integer, intent(in) :: xns_profile_type
...
subroutine mod_XNS_get_profile(lprofile_name, Nr, Nth, rho, press, v3, psi, alp, beta3, &
  ...
  select case(xns_profile_type) 
  case(1)  ! No b field case
  ...
  case(2)  ! Hydroeq.dat but with pure torodial B stored in the same file Hydroeq.dat
  ...
  case(3) ! Mixed B field output, stored in two different files
     ...
     filename = trim(adjustl(lprofile_name))//"/Hydroeq_mag.dat"
        ...
        close(666)
  case default 
     call mpistop('pls select xns_profile_type')
  end select
