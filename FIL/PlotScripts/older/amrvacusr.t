...
module Init_c2b_cart_3d_interface
    ...
    subroutine init_c2b_3d_profile(fpath, fnbase_list, dsname_list, num_refine_levels, num_datablocks, iter_num, &
            save_cpu_or_mem, init_magnetic_field, init_B_from_vecA, use_eps_init)
        use mod_eos
        character(len=256), intent(in)                :: fpath ! path to directory where hdf5 files are stored
        character(len=256), dimension(19), intent(in) :: fnbase_list ! name of the file that store the variable
        character(len=256), dimension(19), intent(in) :: dsname_list ! name of the variable in the attribute
        integer, intent(in)                           :: num_refine_levels, num_datablocks, iter_num, save_cpu_or_mem
        logical, intent(in)                           :: init_magnetic_field, init_B_from_vecA, use_eps_init
...
module NSimport
    ...
    double precision                  :: shift_origin         = 0.0d0       ! shift origin when initializing
      ...
      double precision, parameter       :: r_entire             = 15.0d0 !30.0d0  ! radius that must cover the entire star
      double precision, parameter       :: r_outer              = 20.0d0  ! Outside the radius that refinement lv would be the lowest
    ...
    integer                           :: iteration_num = 200448 ! the iteration number you want to read, before reading, please use h5ls to check what iteration you want exists in that file
    

        ! list of strings that contain the name of the saved variables, like 'HYDROBASE::rho', "HYDROBASE::vel[1]", and so on
    character(len=256), dimension(19) :: variable_name_list =  [ character(len=256):: "HYDROBASE::rho", "HYDROBASE::vel[0]", &
    ...
    character(len=256), dimension(19) :: variable_fname_list = [ character(len=256):: "rho", "vel[0]", &
        "vel[1]", "vel[2]", "W_z4c", "alp", "betax", "betay", "betaz", "temperature", 'Y_e', 'eps', 'P', &
        "Ax", "Ay", "Az", "Bvec[0]", "Bvec[1]", "Bvec[2]"]

    integer                           :: num_ref_levels = 6           ! number of refinement levels, to check this, you can use h5ls to check how many refinement levels are there
    integer                           :: num_data_blocks = 80        ! number of blocks that the data have been separated into
    ...
    logical                           :: use_eps_init = .true.        ! if there is no eps, temperature should be provide in the case of realistic eos, and should provide pressure in other eos cases
    character*256                     :: interp_fluid = 'Linear', interp_metric='Linear', interp_vectA = 'Linear' ! allowed: 'Linear', 'Hermitian3', "Lagrangian2", "Lagrangian3", "Lagrangian4", "Lagrangian5", "Automatic"
...
subroutine initonegrid_usr(ixI^L, ixO^L, s)
  ...
  {#IFDEF M1     
     ! here would need to read fluid distribution on grid
     ! and from that get rates and get distribution of E, F, n
   {^KSP&
     {do ix^D=ixImin^D, ixImax^D \}    
            w(ix^D,nrad^KSP_) = 1.0d-30!1.0d-10  !0.0d0
            w(ix^D,erad^KSP_) = 1.0d-30!1.0d-10  ! 0.0d0
            w(ix^D,frad^KSP1_) = 1.0d-30!0.0d0
            w(ix^D,frad^KSP2_) = 1.0d-30!0.0d0
            w(ix^D,frad^KSP3_) = 1.0d-30!0.0d0
   {end do^D&\}
    !---------------------
    \}
  }
...
subroutine specialvar_output(ixI^L,ixO^L,nwmax,w,s,normconv,dxgrid,level,sold)
...
! corresponding normalization values (default value 1)
    ...
    use mod_metric, only: lower3, lower4, square3u_dysp, get_Kij_dysp
    use mod_cfc, only: cfc_get_Aij_grid
    ...
    double precision                   :: normconv(0:nwmax), b2_new(ixI^S), gamma(ixI^S,1:3,1:3)
    double precision, intent(in)       :: dxgrid(ixI^S,1:ndim)
    integer, intent(in)                :: level
    ! .. local ..
    integer                            :: ix^D, idims
    double precision, dimension(ixI^S) :: b2_comoving
    double precision, dimension(ixI^S) :: B2_eulerian
    double precision, dimension(ixI^S,0:ndir) :: u4, u4d
    double precision, dimension(ixG^T)        :: rhoh
    double precision                   :: xi(ixI^S, 1:ndim), wi(ixI^S, 1:nw)
    double precision                   :: psi6(ixI^S), mm(ixI^S)
    double precision                   :: psi6_surfC^D(ixI^S)
    double precision                   :: Kij(ixI^S, 1:3, 1:3), acce_euler(ixI^S, 1:ndir)
    double precision                   :: aux_A(ixI^S, 1:ndir), aux_lambda(ixI^S, 1:ndir)
    double precision                   :: expansion(ixI^S), aux_Lambda_s(ixI^S), spacial_expansion(ixI^S)
    integer                            :: i, j, max_idx
    !-----------------------------------------------------------------------------
    ...
    {#IFDEF STAGGERED
        call div_staggered(ixI^L, ixO^L, s, w(ixI^S, max_idx+1))
    }{#IFNDEF STAGGERED
        ! Reduce output array size, +1 was added for eventual pointdata output
        call get_divb(ixI^L, ixO^L^LSUB1, w(ixI^S,1:nw), w(ixI^S, max_idx+1))
    }

    w(ixI^S, max_idx+2) = dble(level)
    max_idx = nw+2

    if (extra_2d_output) then
        if (nwmax-max_idx .lt. 1) call mpistop("Variable number incorrect in 2d output!")
        call imhd_cal_current_mag(ixI^L, ixO^L, w(ixI^S,1:nw), x, w(ixI^S, max_idx+1))
        max_idx = max_idx+1

        if (use_t_dependent_output) then 
            !if (mype==0) write(*, *) "debug: ", it, dt, dt_old
            if (nwmax-max_idx .lt. 5) call mpistop("Variable number incorrect in t dependent output!")
            call get_Kij_dysp(ixI^L, ixO^L, w(ixI^S,1:nw), x, Kij)
            call four_accelaration_euler_observer(ixI^L, ixO^L, w(ixI^S,1:nw), x, acce_euler)
            call auxiliary_A_and_lambda(ixI^L, ixO^L, w(ixI^S,1:nw), wold, x, acce_euler, aux_A, aux_lambda)
            call expansion_scalar(ixI^L, ixO^L, w(ixI^S,1:nw), wold, x, acce_euler, expansion, aux_Lambda_scalar=aux_Lambda_s, spacial_expansion=spacial_expansion)

            w(ixO^S, max_idx+1) = expansion(ixO^S)

            call source_mag(ixI^L, ixO^L, w(ixI^S,1:nw), x, Kij, aux_A, aux_lambda, aux_Lambda_s, expansion, spacial_expansion,w(ixI^S,max_idx+2)) ! s_mag

            call vorticity_spacial_projection_tensor(ixI^L, ixO^L, w(ixI^S,1:nw), x, Kij, aux_A, aux_lambda, 1, 2, w(ixI^S,max_idx+3))

            call vorticity_spacial_projection_tensor(ixI^L, ixO^L, w(ixI^S,1:nw), x, Kij, aux_A, aux_lambda, 1, 3, w(ixI^S,max_idx+4))

            call vorticity_spacial_projection_tensor(ixI^L, ixO^L, w(ixI^S,1:nw), x, Kij, aux_A, aux_lambda, 2, 3, w(ixI^S,max_idx+5))
        endif
    endif
    {#IFNDEF STAGGERED
      {#IFDEF DY_SP
 ...
 subroutine specialvarnames_output
    ...
    if (extra_2d_output) then
        write(primnames,"(a,a)") trim(primnames),' Jmag'
        write(wnames,"(a,a)") trim(wnames),' Jmag'
        if (use_t_dependent_output) then
            write(primnames,"(a,a)") trim(primnames),' expS'
            write(wnames,"(a,a)") trim(wnames),' expS'
            write(primnames,"(a,a)") trim(primnames),' s_mag'
            write(wnames,"(a,a)") trim(wnames),' s_mag'
            write(primnames,"(a,a)") trim(primnames),' vort_xy'
            write(wnames,"(a,a)") trim(wnames),' vort_xy'
            write(primnames,"(a,a)") trim(primnames),' vort_xz'
            write(wnames,"(a,a)") trim(wnames),' vort_xz'
            write(primnames,"(a,a)") trim(primnames),' vort_yz'
            write(wnames,"(a,a)") trim(wnames),' vort_yz'
        endif
    endif
...
subroutine printlog_special
    ...
    double precision     :: em_energy_tor_local, em_energy_tor
    double precision     :: em_energy_pol_local, em_energy_pol
    ...
    logical              :: fileopen
    integer              :: iigrid, igrid, level, iw, i, idir, jdir, kdir
    ...
    double precision, dimension(ixG^T, 1:igridstail) :: bsq_comoving, E_em_eul, dvolume, E_em_eul_tor, E_kinetic, E_internal, E_em_eul_pol
    ...
    do iigrid = 1, igridstail
        ...
        ! now does not have eps_, you need to think by yourself, by avoid to do rootfinding
        ...
        call imhd_get_em_energy_pt_decomp(ixG^LL, ixM^LL, pw(igrid)%w(ixG^T, 1:nw), px(igrid)%x(ixG^T, 1:ndim), &
            E_em_eul_pol(ixG^T, iigrid), E_em_eul_tor(ixG^T, iigrid), center_mass_x, center_mass_y)
        ...
        ! calculate Iij_dot
        !{^C& Wv(ixM^T, ^C) = pw(igrid)%w(ixM^T,u^C_) / lfac(ixM^T) \}

        !kdir = 1
        !do idir = 1, 3; do jdir = idir, 3
        !   Iij_dot_local(kdir) = Iij_dot_local(kdir) + &
        !      sum(  pw(igrid)%w(ixM^T,D_) * dvolume(ixM^T,iigrid) /&
        !            lfac(ixM^T) * (  Wv(ixM^T,idir)*px(igrid)%x(ixM^T,jdir) &
        !                           + Wv(ixM^T,jdir)*px(igrid)%x(ixM^T,idir) ) )
        !   kdir = kdir + 1
        !end do; end do
    ...
    call MPI_ALLREDUCE(kinetic_energy_local, kinetic_energy, 1, mpi_double_precision, &
          MPI_SUM, icomm, ierrmpi)
    !call MPI_ALLREDUCE(Iij_dot_local, Iij_dot, 6, mpi_double_precision, &
    !      MPI_SUM, icomm, ierrmpi)
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
        ...
        if (slab) then
           dvolume(ixM^T, iigrid)={rnode(rpdx^D_,igrid)|*}
        else
           dvolume(ixM^T, iigrid)=pgeo(igrid)%dvolume(ixM^T)
           volume(level)=volume(level)+sum(dvolume(ixM^T, iigrid))
        end if
        do iw=1,nw
           wmean(iw)=wmean(iw)+sum(dvolume(ixM^T, iigrid)*pw(igrid)%w(ixM^T,iw))
        end do
    end do
    ...
    if (mype==0) then
        ...
        if (.not.opened) then
            ...
            if (snapshotini==-1) then
                ...
                line_hbase = "rho_max, T_eps_max, ye_min, lfac_max, alp_min, psi_max, h_00_max, delta_max, Q2_11, Q2_12, Q2_13, Q2_22, Q2_23, Q2_33, divB_max, &
                    Dpsi6_max, rest_mass, adm_mass, int_energy, em_energy_tot, em_energy_pol, em_energy_tor, em_energy_comv, kinetic_energy, cmx, cmy"
                do i=1, mxnest
                    write(string_i, *) i
                ...
                if (time_accurate) then
                    if(residmin>smalldouble) then
                        write(line_h,*) "it, t, dt, residual, "//trim(line_hbase)
                    else
                        write(line_h,*) "it, t, dt, "//trim(line_hbase)
                    endif
                else
                    if(residmin>smalldouble) then
                        write(line_h,*) "it, residual, "//trim(line_hbase)
                    else
                        write(line_h,*) "it"
                    endif
                endif
        ...
        if (time_accurate) then
            if(residmin>smalldouble) then
                write(line,*)it, t, dt, residual, rho_max_out, T_eps_max_out, ye_min_out, lfac_max_out, alp_min_out, psi_max_out, &
                 h_00_max_out, delta_max_out, Q2_ij(1:6), divB_max_out, Dpsi6_max, &
                 rest_mass, adm_mass, int_energy, em_energy, em_energy_pol, em_energy_tor, em_energy_comv, kinetic_energy, &
                 center_mass_x, center_mass_y, nleafs_level(1:mxnest)
            else
            ...
            if(residmin>smalldouble) then
                write(line,*) it, residual, rho_max_out, T_eps_max_out, ye_min_out, lfac_max_out, alp_min_out, psi_max_out, &
                 h_00_max_out, delta_max_out, Q2_ij(1:6), divB_max_out, Dpsi6_max, &
                 rest_mass, adm_mass, int_energy, em_energy, em_energy_pol, em_energy_tor, em_energy_comv, kinetic_energy, &
                 center_mass_x, center_mass_y, nleafs_level(1:mxnest)
            else
