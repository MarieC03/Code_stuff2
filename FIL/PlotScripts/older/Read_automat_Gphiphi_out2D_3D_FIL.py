...
# if one should do gyx=gxy etc:
present_axisymmetry = True

Flag_Save_Plots = 1
Flag_Dont_Plot = 0
Flag_no_2D_Plots = 0

Flag_Compute_Rotation = 1
Flag_Compute_Rotation_2DPlot = 1
...
if(present_3D):
    print("---- read in 3D data: -----")
    rho_xyz = gf_xyz['rho']
    print("read in rho data")
    #rho_xy = rho_xyz.sliced([None,None,0.0])
    # not here as it is missing, do this below in timeloop
    #rho_3D_xyz = rho_xyz.read_on_grid(it, grid_3D, resample=True)
    #RhoG_np = rho_3D_xyz.data_xyz # same as above but raw numpy 
    #RhoG_np_2d =RhoG_np[:,:,0] 
    #rho_2D_xy = gd.UniformGridData(grid_2D, RhoG_np_2d)

    if(Flag_press==1):
        press_xyz = gf_xyz['press']
        #press_xyz = gf_xyz['P']
        print("read in press data: done")
    if(Flag_temp==1):
        #temp_xyz = gf_xyz['temperature']
        temp_xyz = gf_xyz['temp']
        print("read in temp data: done")
    if(Flag_eps ==1):
        eps_xyz = gf_xyz['eps']
        print("read in eps data: done")
    if(Flag_entropy==1):
        entropy_xyz = gf_xyz['entropy']
        print("read in entropy data: done")
    
    #--------------------------
    # quantities needed for rotation:
    alpha_xyz = gf_xyz['alp']
    print("read in alpha data: done")
    
    betax_xyz = gf_xyz['betax']
    betay_xyz = gf_xyz['betay']
    betaz_xyz = gf_xyz['betaz'] 
    print("read in beta_i data: done")
    
    ### !!!! 3D metric todo
    gxx_xyz = gf_xyz['gxx']
    gxy_xyz = gf_xyz['gxy']
    gxz_xyz = gf_xyz['gxz'] 
    print("read in gxi data: done")

    if(present_axisymmetry == 0):     
        gyx_xyz = gf_xyz['gxy']
    gyy_xyz = gf_xyz['gyy']  
    gyz_xyz = gf_xyz['gyz'] 
    print("read in gyi data: done")
    
    if(present_axisymmetry == 0): 
        gzx_xyz = gf_xyz['gxz'] 
        gzy_xyz = gf_xyz['gyz']
    gzz_xyz = gf_xyz['gzz']  
    print("read in gzi data: done")
    
    vel0_xyz = gf_xyz['vel[0]']
    vel1_xyz = gf_xyz['vel[1]']
    vel2_xyz = gf_xyz['vel[2]']
    print("read in vel_i data: done")
    #--------------------------
    
    if(Flag_Ye==1):
        Ye_xyz = gf_xyz['Y_e']
        print("read in Ye data: done")
    
    if(magfields == 1):
        Bvec0_xyz = gf_xyz['Bvec[0]']
        Bvec1_xyz = gf_xyz['Bvec[1]']
        Bvec2_xyz = gf_xyz['Bvec[2]']
        print("read in B_i data: done")
    
    # M1 quantities
    if(M1_scheme == 1):
        print("read in 3D M1 data------")
        if(Flag_M1_Enu==1):
            Enua_xyz = gf_xyz['Enue_bar']
            Enue_xyz = gf_xyz['Enue']
            Enux_xyz = gf_xyz['Enux']
            print("read in M1: Enu data: done")
        if(Flag_M1_eps_nu==1):
            eps_nua_xyz = gf_xyz['eps_nue_bar']
            eps_nue_xyz = gf_xyz['eps_nue']
            eps_nux_xyz = gf_xyz['eps_nux']
            print("read in M1: eps data: done")
        if(Flag_M1_eta_nu==1):       
            eta_nua_xyz = gf_xyz['eta_nue_bar']
            eta_nue_xyz = gf_xyz['eta_nue']
            eta_nux_xyz = gf_xyz['eta_nux']
            print("read in M1: eta data: done")
        if(Flag_M1_kappa_a_nu==1):       
            kappa_a_nua_xyz = gf_xyz['kappa_nue_bar_a']
            kappa_a_nue_xyz = gf_xyz['kappa_nue_a']
            kappa_a_nux_xyz = gf_xyz['kappa_nux_a']
            print("read in M1: kappa_a data: done")
        if(Flag_M1_kappa_s_nu==1):       
            kappa_s_nua_xyz = gf_xyz['kappa_nue_bar_s']
            kappa_s_nue_xyz = gf_xyz['kappa_nue_s']
            kappa_s_nux_xyz = gf_xyz['kappa_nux_s']
            print("read in M1: kappa_s data: done")
        if(Flag_M1_Q_nu==1):       
            Q_nua_xyz = gf_xyz['Qnue_bar']
            Q_nue_xyz = gf_xyz['Qnue']
            Q_nux_xyz = gf_xyz['Qnux']
            print("read in M1: Q_nu data: done")
        if(Flag_M1_R_nu==1):       
            R_nua_xyz = gf_xyz['Qnue_bar']
            R_nue_xyz = gf_xyz['Qnue']
            R_nux_xyz = gf_xyz['Qnux']
            print("read in M1: R_nu data: done")

#************************************************
if(present_2D):
    print("read in 2D data")
    rho_xy = gf_xy['rho']
    if(Flag_press==1):
...
if(present_3D):
    print(rho_xyz.available_iterations)
if(present_2D):    
...
j=0
if(present_3D):
    for ii in rho_xyz.available_iterations:
        itera[j] = ii #rho_xy.available_iterations[j]
        #print(itera[j])
        if(itera[j]==it_merger):
            jmerger = j  
            print("j-merger=",jmerger,"with iter=",itera[j])
        tmerger = rho_xyz.time_at_iteration(it_merger)*0.0049267398258
        t_curr = rho_xyz.time_at_iteration(itera[j])*0.0049267398258-tmerger
...
while i<=imax:
    ...
    if(present_3D):
        ...
        print("At iteration",it," Time in Msun:",rho_xyz.time_at_iteration(it))
        ...
        tmerger = rho_xyz.time_at_iteration(it_merger)*0.0049267398258
        t_curr = rho_xyz.time_at_iteration(it)*0.0049267398258-tmerger
    ...
    if(present_3D):
        ...
        rho_2D_xy = gd.UniformGridData(grid_2D, RhoG_np_2d)
        if(Flag_press == 1):
            press_3D_xyz = press_xyz.read_on_grid(it, grid_3D, resample=True)
            PressG_np = press_3D_xyz.data_xyz # same as above but raw numpy 
            PressG_np_2d =PressG_np[:,:,0] 
            press_2D_xy = gd.UniformGridData(grid_2D, PressG_np_2d)
        if(Flag_temp == 1):
            temp_3D_xyz = temp_xyz.read_on_grid(it, grid_3D, resample=True)
            TempG_np = temp_3D_xyz.data_xyz # same as above but raw numpy 
            TempG_np_2d =TempG_np[:,:,0] 
            temp_2D_xy = gd.UniformGridData(grid_2D, TempG_np_2d)
        #eps_3D_xyz = eps_xyz.read_on_grid(it, grid_3D, resample=True)
        if(Flag_entropy == 1):
            entropy_3D_xyz = entropy_xyz.read_on_grid(it, grid_3D, resample=True)
            EntropyG_np = entropy_3D_xyz.data_xyz # same as above but raw numpy 
            EntropyG_np_2d =EntropyG_np[:,:,0] 
            entropy_2D_xy = gd.UniformGridData(grid_2D, EntropyG_np_2d)   
        if(Flag_Ye == 1):
            Ye_3D_xyz = Ye_xyz.read_on_grid(it, grid_3D, resample=True)
            YeG_np = Ye_3D_xyz.data_xyz # same as above but raw numpy 
            YeG_np_2d = YeG_np[:,:,0] 
            Ye_2D_xy = gd.UniformGridData(grid_2D, YeG_np_2d)

        #rho_it = rho_xyz[it]
        alpha_3D_xyz = alpha_xyz.read_on_grid(it, grid_3D, resample=True)
        AlphaG_np = alpha_3D_xyz.data_xyz # same as above but raw numpy 
        AlphaG_np_2d = AlphaG_np[:,:,0] 
        alpha_2D_xy = gd.UniformGridData(grid_2D, AlphaG_np_2d)
        
        betax_3D_xyz = betax_xyz.read_on_grid(it, grid_3D, resample=True)
        BetaxG_np = betax_3D_xyz.data_xyz # same as above but raw numpy 
        BetaxG_np_2d = BetaxG_np[:,:,0] 
        betax_2D_xy = gd.UniformGridData(grid_2D, BetaxG_np_2d)

        betay_3D_xyz = betay_xyz.read_on_grid(it, grid_3D, resample=True)
        BetayG_np = betay_3D_xyz.data_xyz # same as above but raw numpy 
        BetayG_np_2d = BetayG_np[:,:,0] 
        betay_2D_xy = gd.UniformGridData(grid_2D, BetayG_np_2d)

        betaz_3D_xyz = betaz_xyz.read_on_grid(it, grid_3D, resample=True)
        BetazG_np = betaz_3D_xyz.data_xyz # same as above but raw numpy 
        BetazG_np_2d = BetazG_np[:,:,0] 
        betaz_2D_xy = gd.UniformGridData(grid_2D, BetazG_np_2d)

        gxx_3D_xyz = gxx_xyz.read_on_grid(it, grid_3D, resample=True)
        GxxG_np = gxx_3D_xyz.data_xyz # same as above but raw numpy 
        GxxG_np_2d = GxxG_np[:,:,0] 
        gxx_2D_xy = gd.UniformGridData(grid_2D, GxxG_np_2d)

        gxy_3D_xyz = gxy_xyz.read_on_grid(it, grid_3D, resample=True)
        GxyG_np = gxy_3D_xyz.data_xyz # same as above but raw numpy 
        GxyG_np_2d = GxyG_np[:,:,0] 
        gxy_2D_xy = gd.UniformGridData(grid_2D, GxyG_np_2d)

        gxz_3D_xyz = gxz_xyz.read_on_grid(it, grid_3D, resample=True)
        GxzG_np = gxz_3D_xyz.data_xyz # same as above but raw numpy 
        GxzG_np_2d = GxzG_np[:,:,0] 
        gxz_2D_xy = gd.UniformGridData(grid_2D, GxzG_np_2d)
    
        if(present_axisymmetry):
            gyx_2D_xy = gd.UniformGridData(grid_2D, GxyG_np_2d)
        else:
            gyx_3D_xyz = gxy_xyz.read_on_grid(it, grid_3D, resample=True)
            GyxG_np = gxy_3D_xyz.data_xyz # same as above but raw numpy 
            GyxG_np_2d = GyxG_np[:,:,0] 
            gyx_2D_xy = gd.UniformGridData(grid_2D, GyxG_np_2d)

        gyy_3D_xyz = gyy_xyz.read_on_grid(it, grid_3D, resample=True)
        GyyG_np = gyy_3D_xyz.data_xyz # same as above but raw numpy 
        GyyG_np_2d = GyyG_np[:,:,0] 
        gyy_2D_xy = gd.UniformGridData(grid_2D, GyyG_np_2d)

        gyz_3D_xyz = gyz_xyz.read_on_grid(it, grid_3D, resample=True)
        GyzG_np = gyz_3D_xyz.data_xyz # same as above but raw numpy 
        GyzG_np_2d = GyzG_np[:,:,0] 
        gyz_2D_xy = gd.UniformGridData(grid_2D, GyzG_np_2d)

        if(present_axisymmetry):
            gzx_2D_xy = gd.UniformGridData(grid_2D, GxzG_np_2d)
        else:
            gzx_3D_xyz = gzx_xyz.read_on_grid(it, grid_3D, resample=True)
            GzxG_np = gzx_3D_xyz.data_xyz # same as above but raw numpy 
            GzxG_np_2d = GzxG_np[:,:,0] 
            gzx_2D_xy = gd.UniformGridData(grid_2D, GzxG_np_2d)

        if(present_axisymmetry):
            gzy_2D_xy = gd.UniformGridData(grid_2D, GyzG_np_2d)
        else:
            gzy_3D_xyz = gzy_xyz.read_on_grid(it, grid_3D, resample=True)
            GzyG_np = gzy_3D_xyz.data_xyz # same as above but raw numpy 
            GzyG_np_2d = GzyG_np[:,:,0] 
            gzy_2D_xy = gd.UniformGridData(grid_2D, GzyG_np_2d)

        gzz_3D_xyz = gzz_xyz.read_on_grid(it, grid_3D, resample=True)
        GzzG_np = gzz_3D_xyz.data_xyz # same as above but raw numpy 
        GzzG_np_2d = GzzG_np[:,:,0] 
        gzz_2D_xy = gd.UniformGridData(grid_2D, GzzG_np_2d)

        vel0_3D_xyz = vel0_xyz.read_on_grid(it, grid_3D, resample=True)
        vel0G_np = vel0_3D_xyz.data_xyz # same as above but raw numpy 
        vel0G_np_2d = vel0G_np[:,:,0] 
        vel0_2D_xy = gd.UniformGridData(grid_2D, vel0G_np_2d)

        vel1_3D_xyz = vel1_xyz.read_on_grid(it, grid_3D, resample=True)
        vel1G_np = vel1_3D_xyz.data_xyz # same as above but raw numpy 
        vel1G_np_2d = vel1G_np[:,:,0] 
        vel1_2D_xy = gd.UniformGridData(grid_2D, vel1G_np_2d)

        vel2_3D_xyz = vel2_xyz.read_on_grid(it, grid_3D, resample=True)
        vel2G_np = vel2_3D_xyz.data_xyz # same as above but raw numpy 
        vel2G_np_2d = vel2G_np[:,:,0] 
        vel2_2D_xy = gd.UniformGridData(grid_2D, vel2G_np_2d)

        if(magfields == 1):
            print("3D Bfield not supported yet")
            Bvec0_3D_xyz = Bvec0_xyz.read_on_grid(it, grid_3D, resample=True)
            Bvec1_3D_xyz = Bvec0_xyz.read_on_grid(it, grid_3D, resample=True)
            Bvec2_3D_xyz = Bvec0_xyz.read_on_grid(it, grid_3D, resample=True)
    
        if(M1_scheme):
            print("3D M1 not supported yet")
            if(Flag_M1_Enu==1):
                Enua_3D_xyz = Enua_xyz.read_on_grid(it, grid_3D, resample=True)
                Enue_3D_xyz = Enue_xyz.read_on_grid(it, grid_3D, resample=True)
                Enux_3D_xyz = Enux_xyz.read_on_grid(it, grid_3D, resample=True)
            if(Flag_M1_eps_nu==1):
                eps_nua_3D_xyz = eps_nua_xyz.read_on_grid(it, grid_3D, resample=True)
                eps_nue_3D_xyz = eps_nue_xyz.read_on_grid(it, grid_3D, resample=True)
                eps_nux_3D_xyz = eps_nux_xyz.read_on_grid(it, grid_3D, resample=True)
            if(Flag_M1_eta_nu==1):
                eta_nua_3D_xyz = eta_nua_xyz.read_on_grid(it, grid_3D, resample=True)
                eta_nue_3D_xyz = eta_nue_xyz.read_on_grid(it, grid_3D, resample=True)
                eta_nux_3D_xyz = eta_nux_xyz.read_on_grid(it, grid_3D, resample=True)
            if(Flag_M1_kappa_a_nu==1):
                kappa_a_nua_3D_xyz = kappa_a_nua_xyz.read_on_grid(it, grid_3D, resample=True)
                kappa_a_nue_3D_xyz = kappa_a_nue_xyz.read_on_grid(it, grid_3D, resample=True)
                kappa_a_nux_3D_xyz = kappa_a_nux_xyz.read_on_grid(it, grid_3D, resample=True)
            if(Flag_M1_kappa_s_nu==1):
                kappa_s_nua_3D_xyz = kappa_a_nua_xyz.read_on_grid(it, grid_3D, resample=True)
                kappa_s_nue_3D_xyz = kappa_a_nue_xyz.read_on_grid(it, grid_3D, resample=True)
                kappa_s_nux_3D_xyz = kappa_a_nux_xyz.read_on_grid(it, grid_3D, resample=True)
            if(Flag_M1_Q_nu==1):
                Q_nua_3D_xyz = Q_nua_xyz.read_on_grid(it, grid_3D, resample=True)
                Q_nue_3D_xyz = Q_nue_xyz.read_on_grid(it, grid_3D, resample=True)
                Q_nux_3D_xyz = Q_nux_xyz.read_on_grid(it, grid_3D, resample=True)        
    if(present_2D):
   ...
   #     Om1x_df.to_csv(Filename1,header = False, index= False)
    ...
    ################################################################
    ###############################################################################
    if(Flag_Compute_Rotation_2DPlot):
        ######################################## W lorentz 2D ###################################
        fig = plt.figure(figsize=(6,6), dpi=128)
        ax = fig.add_subplot()
         #ax = fig.add_subplot(projection='3d', computed_zorder=True)
