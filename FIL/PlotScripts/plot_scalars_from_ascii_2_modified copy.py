...
def remove_collapse_spike(x: np.ndarray, y: np.ndarray, tcoll: float,
                          window_ms: float = 3.0,
                          log10_excess: float = 0.15) -> np.ndarray:
    """Mask out a broad anomalous spike in y near the collapse time.

    Points within [tcoll - window_ms, tcoll + window_ms] that exceed
    the log-linear interpolation between the pre- and post-window
    medians by more than log10_excess orders of magnitude are removed.
    This handles multi-point artifacts that remove_isolated_log_spikes misses.

    Parameters
    ----------
    x, y        : data arrays (already filtered for finite/positive values upstream)
    tcoll       : collapse time in the same units as x
    window_ms   : half-width of the masking window around tcoll
    log10_excess: threshold above interpolated log10(y) to flag as spike
    """
    x = np.asarray(x, dtype=float)
    y = np.asarray(y, dtype=float)
    keep = np.isfinite(x) & np.isfinite(y) & (y > 0.0)
    ...
    before = keep & (x < tcoll - window_ms)
    after  = keep & (x > tcoll + window_ms)

    if not np.any(before) or not np.any(after):
        return keep  # cannot estimate baseline, leave untouched

    # Use the last/first few points outside the window to estimate baseline
    n_ref = 20
    idx_before = np.where(before)[0][-n_ref:]
    idx_after  = np.where(after)[0][:n_ref]

    log_y_pre  = np.median(np.log10(y[idx_before]))
    log_y_post = np.median(np.log10(y[idx_after]))
    x_pre      = np.median(x[idx_before])
    x_post     = np.median(x[idx_after])

    if x_post <= x_pre:
        return keep  # degenerate range, skip

    for i in np.where(in_window & keep)[0]:
        t = (x[i] - x_pre) / (x_post - x_pre)
        expected_log = log_y_pre + t * (log_y_post - log_y_pre)
        if np.log10(y[i]) > expected_log + log10_excess:
            keep[i] = False
...
def plot_series_compare(df_m1: pd.DataFrame, df_nom1: pd.DataFrame,
                        ...
                        tcoll_spike_window_ms: float = 3.0,
                        tcoll_spike_log10_excess: float = 0.15) -> bool:
        ...
        if remove_tcoll_spike:
            if tcoll_m1_plot is not None and x1.size > 0:
                m1 = m1 & remove_collapse_spike(x1, y1, tcoll_m1_plot,
                                                window_ms=tcoll_spike_window_ms,
                                                log10_excess=tcoll_spike_log10_excess)
...
def main():
    ...
    created_any |= plot_series_compare(df_epol_m1, df_epol_nom1, "value", "value", r"$E_{\rm mag,pol}~[{\rm erg}]$", tmerg_m1_ms, tmerg_nom1_ms, outdir, prefix, "E_magPOL_compare_M1_noM1", xlabel, args.tmin, tcoll_m1_plot, tcoll_nom1_plot, CU_to_energyCGS, CU_to_energyCGS, True, None, colour_m1=args.colour_M1, colour_nom1=args.colour_noM1, remove_tcoll_spike=True, tcoll_spike_window_ms=3.0, tcoll_spike_log10_excess=0.15)
