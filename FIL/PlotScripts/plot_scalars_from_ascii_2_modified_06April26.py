...
def plot_scalar_compare(entry_m1: Dict[str, object], entry_nom1: Dict[str, object],
    ...
    if use_log:
        m1 = np.isfinite(y1) & (y1 > 0.0)
        m0 = np.isfinite(y0) & (y0 > 0.0)
...
def plot_series_single(df: pd.DataFrame, ycol: str, ylabel: str,
    ...
    m = (np.isfinite(y) & (y > 0.0)) if logscale else np.isfinite(y)
    if not np.any(m):
        return False
    fig = plt.figure(figsize=FIGSIZE_SQUARE)
    ax = fig.add_subplot(111)
...
def plot_series_compare(df_m1: pd.DataFrame, df_nom1: pd.DataFrame,
    ...
    m1 = (np.isfinite(y1) & (y1 > 0.0)) if logscale else np.isfinite(y1)
    m0 = (np.isfinite(y0) & (y0 > 0.0)) if logscale else np.isfinite(y0)
    if not np.any(m1) and not np.any(m0):
        return False
    fig = plt.figure(figsize=FIGSIZE_SQUARE)
    ax = fig.add_subplot(111)
...
def plot_neutrino_luminosities_m1(df_lnue: pd.DataFrame, df_lnua: pd.DataFrame, df_lnux: pd.DataFrame,
    ...
    with np.errstate(over='ignore', invalid='ignore'):
        ...
        Lnu = Lnue + Lnua + Lnux
    m_tot = np.isfinite(Lnu) & (Lnu > 0.0)
    m_nue = np.isfinite(Lnue) & (Lnue > 0.0)
    m_nua = np.isfinite(Lnua) & (Lnua > 0.0)
    m_nux = np.isfinite(Lnux) & (Lnux > 0.0)
    ...
    ax.legend(fontsize=13)
    finalize_axes(ax, xlabel, r'$L_\nu~[{\rm erg/s}]$', [x_nue[m_tot] if np.any(m_tot) else np.array([])], tmin, (1e51, 2e54), tcoll_m1_plot, None)
    fig.tight_layout()
...
def main():
    ...
    if not args.compare_M1_noM1:
        ...
        if not args.noM1:
            ...
            plot_neutrino_luminosities_m1(df_lnue, df_lnua, df_lnux, tmerg_ms, outdir, prefix, xlabel, args.tmin, tcoll_plot, colour_m1=args.colour_M1)
    ...
    created_any |= plot_neutrino_luminosities_m1(df_lnue_m1, df_lnua_m1, df_lnux_m1, tmerg_m1_ms, outdir, prefix, xlabel, args.tmin, tcoll_m1_plot, colour_m1=args.colour_M1)
