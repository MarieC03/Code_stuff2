...
def remove_collapse_spike(x: np.ndarray, y: np.ndarray, tcoll: float,
                          window_ms: float = 3.0,
                          log10_excess: float = 0.10,
                          ref_width_ms: float = 2.0) -> np.ndarray:
    """Mask only the upward spike near the collapse time, preserving the
    natural post-collapse drop.

    Strategy: use the median log10(y) of points in a *narrow strip*
    immediately before the masking window as a one-sided ceiling.  Using a
    narrow local strip (rather than the full pre-window history) avoids the
    ceiling being inflated by earlier physical peaks that are unrelated to the
    collapse artifact.

    Any point inside the window that exceeds this ceiling by more than
    log10_excess dex is masked.  Points at or below the ceiling (including
    the real physical drop after collapse) are kept untouched, so matplotlib
    draws the post-collapse drop exactly where it belongs.

    Parameters
    ----------
    x, y          : data arrays (positive, finite values expected)
    tcoll         : collapse time in the same units as x
    window_ms     : half-width of the spike-masking window around tcoll
    log10_excess  : dex above the local ceiling to flag as spike (default 0.10)
    ref_width_ms  : width of the local reference strip ending at
                    tcoll - window_ms (default 2.0 ms)
    """
    x = np.asarray(x, dtype=float)
    y = np.asarray(y, dtype=float)
    keep = np.isfinite(x) & np.isfinite(y) & (y > 0.0)

    in_window  = (x >= tcoll - window_ms) & (x <= tcoll + window_ms)
    ref_strip  = keep & (x >= tcoll - window_ms - ref_width_ms) & (x < tcoll - window_ms)

    if not np.any(ref_strip):
        # Fallback: last 5 points before the window
        fallback = keep & (x < tcoll - window_ms)
        if not np.any(fallback):
            return keep
        ref_idx = np.where(fallback)[0][-5:]
        log_y_ceiling = np.median(np.log10(y[ref_idx]))
    else:
        log_y_ceiling = np.median(np.log10(y[ref_strip]))

    # Mask only upward excursions above the local ceiling; drops are kept
    for i in np.where(in_window & keep)[0]:
        if np.log10(y[i]) > log_y_ceiling + log10_excess:
            keep[i] = False
...
def plot_series_compare(df_m1: pd.DataFrame, df_nom1: pd.DataFrame,
                        ...
                        collapse_y_axes: float = 0.98, collapse_va: str = "top",
                        remove_tcoll_spike: bool = False,
                        tcoll_spike_window_ms_m1: float = 4.0,
                        tcoll_spike_log10_excess_m1: float = 0.10,
                        tcoll_spike_ref_ms_m1: float = 2.0,
                        tcoll_spike_window_ms_nom1: float = 3.0,
                        tcoll_spike_log10_excess_nom1: float = 0.10,
                        tcoll_spike_ref_ms_nom1: float = 2.0) -> bool:
    ...
    if logscale:
        m1 = remove_isolated_log_spikes(x1, y1)
        m0 = remove_isolated_log_spikes(x0, y0)
        # Remove broad multi-point spike near collapse if requested
        if remove_tcoll_spike:
            if tcoll_m1_plot is not None and x1.size > 0:
                m1 = m1 & remove_collapse_spike(x1, y1, tcoll_m1_plot,
                                                window_ms=tcoll_spike_window_ms_m1,
                                                log10_excess=tcoll_spike_log10_excess_m1,
                                                ref_width_ms=tcoll_spike_ref_ms_m1)
            if tcoll_nom1_plot is not None and x0.size > 0:
                m0 = m0 & remove_collapse_spike(x0, y0, tcoll_nom1_plot,
                                                window_ms=tcoll_spike_window_ms_nom1,
                                                log10_excess=tcoll_spike_log10_excess_nom1,
                                                ref_width_ms=tcoll_spike_ref_ms_nom1)
    else:
...
def main():
    ...
    created_any |= plot_series_compare(df_epol_m1, df_epol_nom1, "value", "value", r"$E_{\rm mag,pol}~[{\rm erg}]$", tmerg_m1_ms, tmerg_nom1_ms, outdir, prefix, "E_magPOL_compare_M1_noM1", xlabel, args.tmin, tcoll_m1_plot, tcoll_nom1_plot, CU_to_energyCGS, CU_to_energyCGS, True, None, colour_m1=args.colour_M1, colour_nom1=args.colour_noM1, remove_tcoll_spike=True, tcoll_spike_window_ms_m1=4.0, tcoll_spike_log10_excess_m1=0.10, tcoll_spike_ref_ms_m1=2.0, tcoll_spike_window_ms_nom1=3.0, tcoll_spike_log10_excess_nom1=0.10, tcoll_spike_ref_ms_nom1=2.0)
