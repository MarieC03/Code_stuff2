...
namespace frankfurt_m1 {
  ...
  void closure_t::get_number_source(CCTK_REAL &N, CCTK_REAL const& dt, std::array<CCTK_REAL,NUM_EAS> const& eas, CCTK_REAL& dYE) const {
    auto const alp = gamma->lapse ;
  ...
  CCTK_REAL closure_t::get_collisional_rootfinder( std::array<CCTK_REAL, 5> const& eas,
    ...
    */
    ...
    // finally compute ye source term 
    get_number_source(_N, dt, eas, dYe) ;
