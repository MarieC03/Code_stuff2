...
//                  "Hempel_DD2EOS_rho234_temp180_ye60_version_1.1_20120817.h5");
  ...
  auto tau_int= compute_analytic_opacity(rho);//std::array<double,3> {{0,0,0}};
  auto tau_n = std::array<double,3> {{tau_int, tau_int, tau_int}};
  ...
  for(int i=0;i<3;++i){
    nan_found = nan_found || !std::isfinite(lk.kappa_e_cgs(i));
    nan_found = nan_found || !std::isfinite(lk.kappa_n_cgs(i));
  }
  ...
  std::cout << "Not finite count: MargheritaLeak has produced " << nan_count << " non-finite values." <<std::endl; 
