...
//#endif
  ...
  if (Weakhub_table_type != 100) {
        CCTK_ERROR("Weakhub_table is not a grey table!\n") ;
  }
  ...
  if ( (EOS_Leptonic::use_muonic_eos && IVymu <= 1) || EOS_Tabulated::use_muonic_eos && IVymu <= 1) {
    CCTK_ERROR("using muonic eos, but weakhub table is only for npe") ; 
  }
 ...
 // Read the opacity tables
  ...
  for (int i_spec = i_nu_e; i_spec < n_spec; i_spec++) {
    std::cout << "\n kappa_a_max_local\n" << " "<<i_spec<<" "<< std::scientific <<kappa_max_local[i_spec];
    std::cout << "\n kappa_a_min_local\n" << " "<<i_spec<<" "<< std::scientific <<kappa_min_local[i_spec];
  }
