...
&error) {
  ...
  const auto h_max = 1. + exp(epsmax_vec[v_index::EPS]) - energy_shift + press_max/rhomax;
  const auto h_min = 1. + exp(epsmin_vec[v_index::EPS]) - energy_shift + press_min/rhomin;
  ...
  const auto func = [&](const double &lrho) {
   const auto pL = lintp.interpolate(lrho,v_index::PRESS, v_index::EPS);
   return h_cold - ( 1. + (exp(pL[v_index::EPS]) - energy_shift) + exp(pL[v_index::PRESS])/exp(lrho));
  };

  auto lrho = zero_brent<>(log(rho_bound/10.), 0.999*log(rhomax), 1.0e-13, func);
...
template <int extra_vars,template<typename,int> class interp_t>
double Cold_Table_t<extra_vars,interp_t>::baryon_mass;
