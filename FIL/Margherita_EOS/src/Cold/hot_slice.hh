...
void HotSlice_beta_eq(const int downsample_rho, const int num_cold_points,
  ...
  if (EOS_Leptonic::use_muonic_eos) {
     ...
     const auto num_rho_points = std::max(
         4, std::min(static_cast<int>(EOS_Leptonic::alltables_baryon.num_points[0]),
                     downsample_rho));
     const auto delta_rho_initial = (EOS_Leptonic::alltables_baryon.xmax<0>() -
                                     EOS_Leptonic::alltables_baryon.xmin<0>()) /
                                    (num_rho_points - 1);

     // FIXME:: Should we initialize YMU for the vars array as well? we set it to be zero first
     double ymu = 0 ; 
     ...
     for (int i = 0; i < num_rho_points; ++i) {
       auto rhoL = EOS_Leptonic::alltables_baryon.xmin<0>() + i * delta_rho_initial;
       rho_ptr[i] = rhoL;
       rhoL = exp(rhoL);
       EOS_Leptonic::error_type error;
       vars[var_i::PRESS][i] = EOS_Leptonic::press_eps_yle_ymu__beta_eq__rho_temp(vars[var_i::EPS][i], vars[var_i::YE][i], 
                                                                                   ymu, rhoL, temp, error);
       vars[var_i::TEMP][i] = temp;

       epsminL = std::min(vars[var_i::EPS][i], epsminL);

       // Get other auxilliaries
       EOS_Leptonic::eps_csnd2_entropy__temp_rho_yle_ymu(
           vars[var_i::CS2][i], vars[var_i::ENTROPY][i], temp, rhoL,
           vars[var_i::YE][i], ymu, error);
     }

     Hot_Slice::rhomin = EOS_Leptonic::eos_rhomin;
     Hot_Slice::rhomax = EOS_Leptonic::eos_rhomax;
     ...
     if (epsminL < 0) {
       Hot_Slice::energy_shift = -2. * epsminL;
     }
     ...
     // Interpolate
     const auto delta_rho = (EOS_Leptonic::alltables_baryon.xmax<0>() -
                             EOS_Leptonic::alltables_baryon.xmin<0>()) /
                            (num_cold_points - 1);

     for (int i = 0; i < num_cold_points; ++i) {
       auto rhoL = log(EOS_Leptonic::eos_rhomin) + delta_rho * i;
       rho_ptr_lin[i] = rhoL;
  ...
  // start of tabulated eos
     ...
     for (int i = 0; i < num_rho_points; ++i) {
       ...
       vars[var_i::PRESS][i] = EOS_Tabulated::press_eps_yle_ymu__beta_eq__rho_temp(vars[var_i::EPS][i], vars[var_i::YE][i], 
                                                                                   ymu, rhoL, temp, error);
       vars[var_i::TEMP][i] = temp;
       ...
       // Get other auxilliaries
       EOS_Tabulated::eps_csnd2_entropy__temp_rho_yle_ymu(
           vars[var_i::CS2][i], vars[var_i::ENTROPY][i], temp, rhoL,
           vars[var_i::YE][i], ymu, error);
     }
...
void HotSlice_beta_eq_isentropic(const int downsample_rho,
  ...
  if (EOS_Leptonic::use_muonic_eos) {
     ...
     auto num_rho_points = std::min(
         static_cast<int>(EOS_Leptonic::alltables_baryon.num_points[0]), downsample_rho);
     const auto delta_rho_initial = (EOS_Leptonic::alltables_baryon.xmax<0>() -
                                     EOS_Leptonic::alltables_baryon.xmin<0>()) /
                                    (num_rho_points - 1);

     const auto delta_temp = (EOS_Leptonic::alltables_baryon.xmax<1>() -
                              EOS_Leptonic::alltables_baryon.xmin<1>()) /
                             (EOS_Leptonic::alltables_baryon.num_points[1] - 1);
     ...
     auto temp_ptr = std::unique_ptr<double[]>{
         new double[EOS_Leptonic::alltables_baryon.num_points[1]]};
     for (int i = 0; i < var_i::NUM_VARS; ++i)
       vars[i] = std::unique_ptr<double[]>{new double[num_rho_points]};
     ...
     for (int i = 0; i < EOS_Leptonic::alltables_baryon.num_points[1]; ++i) {
       temp_ptr[i] = EOS_Leptonic::alltables_baryon.xmin<1>() + i * delta_temp;
       auto temp = exp(temp_ptr[i]);
       for (int rr = 0; rr < num_rho_points; ++rr) {
         auto rhoL = EOS_Leptonic::alltables_baryon.xmin<0>() + rr * delta_rho_initial;
         rho_ptr[rr] = rhoL;
         rhoL = exp(rhoL);
         auto ye = double{};
         auto eps = double{};
         ...
         EOS_Leptonic::error_type error;
         EOS_Leptonic::press_eps_yle_ymu__beta_eq__rho_temp(eps, ye, ymu, rhoL, temp,
                                                        error);
         auto csnd2 = double{};
         // Now get entropy
         EOS_Leptonic::eps_csnd2_entropy__temp_rho_yle_ymu(csnd2, entropies[rr][i],
           						 temp, rhoL, ye, ymu, error);
       }
     }
     ...
     auto temp_intp =
         cubic_spline_t<double, 1>(EOS_Leptonic::alltables_baryon.num_points[1],
     ...
     const auto maxtemp = EOS_Leptonic::alltables_baryon.xmax<1>();
     const auto mintemp = EOS_Leptonic::alltables_baryon.xmin<1>();
     ...
     for (int i = 0; i < num_rho_points; ++i) {
       ...
       EOS_Leptonic::error_type error;
       vars[var_i::PRESS][i] = EOS_Leptonic::press_eps_yle_ymu__beta_eq__rho_temp(
           									vars[var_i::EPS][i], vars[var_i::YE][i], ymu, rhoL, vars[var_i::TEMP][i],
           error);
       ...
       EOS_Leptonic::eps_csnd2_entropy__temp_rho_yle_ymu(
           ...
           rhoL, vars[var_i::YE][i], ymu, error);
     }
  ...
  // start of tabulated eos
     ...
     for (int i = 0; i < EOS_Tabulated::alltables.num_points[1]; ++i) {
       ...
       for (int rr = 0; rr < num_rho_points; ++rr) {
         ...
         // Compute beta equilibrium
         EOS_Tabulated::error_type error;
         EOS_Tabulated::press_eps_yle_ymu__beta_eq__rho_temp(eps, ye, ymu, rhoL, temp,
                                                        error);
         auto csnd2 = double{};
         // Now get entropy
         EOS_Tabulated::eps_csnd2_entropy__temp_rho_yle_ymu(csnd2, entropies[rr][i],
           						 temp, rhoL, ye, ymu, error);
       }
     }
     ...
     for (int i = 0; i < num_rho_points; ++i) {
       auto rhoL = exp(rho_ptr[i]);
       EOS_Tabulated::error_type error;
       vars[var_i::PRESS][i] = EOS_Tabulated::press_eps_yle_ymu__beta_eq__rho_temp(
           									vars[var_i::EPS][i], vars[var_i::YE][i], ymu, rhoL, vars[var_i::TEMP][i],
           error);
