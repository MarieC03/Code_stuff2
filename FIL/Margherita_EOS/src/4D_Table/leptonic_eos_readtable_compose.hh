...
#ifndef EOS_LEPTONIC_EOS_READTABLE_COMPOSE_HH
#define EOS_LEPTONIC_EOS_READTABLE_COMPOSE_HH

//#ifndef EOS_LEPTONIC_EOS_READTABLE_SCOLLAPSE_HH
//// Catch HDF5 errors
...
void EOS_Leptonic::readtable_compose(const char *nuceos_table_name) {
  using namespace Margherita_constants;

  constexpr size_t NTABLES = EOS_Leptonic::EBARYON::NUM_VARS_BARYON;
  ...
  int status_av = H5Gget_objinfo(file,"Composition_quadrupels",0,nullptr);

  hid_t av_id;

  if(status_av ==0){
    HDF5_ERROR(av_id = H5Gopen(file, "/Composition_quadrupels"));
  ...
  // IMPORTANT: The order here needs to match EBARYON in tabulated.hh !
  ...
  // the public member EOS_Leptonic.
  ...
  // New: pre-subraction electron contributions:
  double pressL, press_b, press_ele, press_pos, lrhoL, ltempL, yeL;
  double epsL, eps_b, eps_ele, eps_pos;
  double entL, ent_b, ent_ele, ent_pos;
  if (presubtract_e_contri) {
        for (int iv = EBARYON::PRESS; iv <= EBARYON::MUN; iv++)
          for (int k = 0; k < nye; k++)
            for (int j = 0; j < ntemp; j++)
              for (int i = 0; i < nrho; i++) {
                ...
                double lrhoL  = logrho[i];
                double ltempL = logtemp[j];
                double yeL    = yes[k];
                // modify pressure with e-e+ contributions
                if (iv == EBARYON::PRESS) {
                  auto const vars_ele = alltables_ele.interpolate<EELE::PRESS_E_MINUS, EELE::PRESS_E_PLUS>(lrhoL, ltempL, yeL);
                  press_ele = vars_ele[0]/ (MeV_to_erg * cm3_to_fm3 * PRESSGF);
                  press_pos = vars_ele[1]/ (MeV_to_erg * cm3_to_fm3 * PRESSGF);
                  press_b   = thermo_table[indold];
                  thermo_table[indold] = Margherita_helpers::max(press_b - press_ele - press_pos, 1e-18/ (MeV_to_erg * cm3_to_fm3 * PRESSGF) );
                }
                // modify eps with e-e+ contributions
                if (iv == EBARYON::EPS) {
                  auto const vars_ele = alltables_ele.interpolate<EELE::EPS_E_MINUS, EELE::EPS_E_PLUS>(lrhoL, ltempL, yeL);
                  eps_ele = vars_ele[0];
                  eps_pos = vars_ele[1];
                  eps_b   = thermo_table[indold];
                  thermo_table[indold] = eps_b - eps_ele - eps_pos;
                }
                // modify S with e-e+ contributions
                if (iv == EBARYON::S) {
                  auto const vars_ele = alltables_ele.interpolate<EELE::S_E_MINUS, EELE::S_E_PLUS>(lrhoL, ltempL, yeL);
                  ent_ele = vars_ele[0];
                  ent_pos = vars_ele[1];
                  ent_b   = thermo_table[indold];
                  thermo_table[indold] = Margherita_helpers::max(ent_b - ent_ele - ent_pos, 0.0);
                }
  ...
  for (int k = 0; k < nye; k++)
	...
	if(nav >0){
	  // ABAR
	  alltables[EBARYON::ABAR + indnew] = aav_table[indold];
	  // ZBAR
	  alltables[EBARYON::ZBAR + indnew] = zav_table[indold];
	  // Xh
	  alltables[EBARYON::XH + indnew] = aav_table[indold] * yav_table[indold];
	}
	if(ncomp>0){
	  // Xn
	  alltables[EBARYON::XN + indnew] =
	      yi_table[indold + nrho * nye * ntemp * find_index_yi(10)];
	  // Xp
	  alltables[EBARYON::XP + indnew] =
	      yi_table[indold + nrho * nye * ntemp * find_index_yi(11)];
	  // Xa
	  alltables[EBARYON::XA + indnew] =
  ...
  for (int i = 0; i < nrho * ntemp * nye; i++) {
    int idx = EOS_Leptonic::EBARYON::EPS + NTABLES * i;
  ...
  for (int i = 0; i < nrho * ntemp * nye; i++) {
    ...
    {  // pressure
      int idx = EOS_Leptonic::EBARYON::PRESS + NTABLES * i;
    ...
    {  // cs2
      int idx = EOS_Leptonic::EBARYON::CS2 + NTABLES * i;
    ...
    {  // chemical potentials
      ...
      int idx_p = EOS_Leptonic::EBARYON::MUP + NTABLES * i;
      int idx_n = EOS_Leptonic::EBARYON::MUN + NTABLES * i;
      int idx_e = EOS_Leptonic::EBARYON::MUE + NTABLES * i;
  ...
  if (eos_yemax != eos_ylemax) {
   std::cout << "\n EOS_Leptonic::eos_yemax != EOS_Leptonic::eos_ylemax ="<< eos_ylemax <<" "<<EOS_Leptonic::eos_yemax<<"\n";
   CCTK_ERROR("EOS_Leptonic::eos_ylemax != EOS_Leptonic::eos_yemax in compose reader");

  }
  if (eos_yemin != eos_ylemin) {
   std::cout << "\n EOS_Leptonic::eos_yemin != EOS_Leptonic::eos_ylemin ="<< eos_ylemin <<" "<<EOS_Leptonic::eos_yemin<<"\n";
   CCTK_ERROR("EOS_Leptonic::eos_ylemin != EOS_Leptonic::eos_yemin in compose reader");
  }
  ...
  EOS_Leptonic::alltables_baryon = linear_interp_uniform_ND_t<double, 3, EBARYON::NUM_VARS_BARYON>(
