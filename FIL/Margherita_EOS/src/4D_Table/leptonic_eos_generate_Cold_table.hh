...
void EOS_Leptonic::generate_cold_table(double logrho[], int &nrho){
  ...
  baryonic_mass = EOS_Leptonic::baryon_mass;
  std::cout <<"\n"<< baryonic_mass <<"_"<<"baryonic mass"<<"\n";

  std::ofstream myfile ("npemu_DD2_T0-5mev.rns");
  myfile << nrho << "\n";
    ...
    for (int i = 0; i < nrho; ++i) {
      ...
      rho_pt = exp(logrho[i]);
      press_pt = EOS_Leptonic::press_eps_yle_ymu__beta_eq__rho_temp(eps_pt, yp_pt, ymu_pt, 
      ...
      //h_pt = c2_cgs * log( (e_pt+press_pt/c2_cgs)/( nb_pt * mnuc_cgs ) );
  ...
  myfile.close();
  CCTK_ERROR("Cold EOS Table for RNS is finished");
