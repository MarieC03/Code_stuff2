...
static double Margherita_Con2Prim(
  ...
  if( eos::has_ymu ) {
     // Set YMU
     PRIMS[YMU] = CONS[YMUSTAR] / CONS[RHOSTAR]; 
  } else {
     PRIMS[YMU] = 0.0;
  }
  ...
  // We need to check whether YE and YMU is within the table bounds
  ...
  if( eos::has_ymu ) {
     if (PRIMS[YMU] < eos::eos_ymumin) {
       error_bits[c2p_errors::YMU_ADJUSTED] = true;
       PRIMS[YMU] = eos::eos_ymumin;
     } else {
       if (PRIMS[YMU] > eos::eos_ymumax) {
         error_bits[c2p_errors::YMU_ADJUSTED] = true;
         PRIMS[YMU] = eos::eos_ymumax;
       }
     }
  }
