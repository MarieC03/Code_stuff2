...
int main() {
  ...
  nue_file.open("nue_weakrates_T10.txt");
  nua_file.open("nua_weakrates_T10.txt");
  nux_file.open("nux_weakrates_T10.txt");
  ...
  double temp = 10;//16.852590447507527;
  std::vector<double> rhos ;
  const int  npoints = 10000;
  generate_logspace(1e-04,1e-02,npoints,rhos);
  ...
  for(auto& rho : rhos){
    ...
    for(int i=0;i<3;++i){
      ...
      (*outfiles[i]) << eas.kappa_s_cactus(i) << '\t' ;
      (*outfiles[i]) << eas.kappa_a_cactus(i) << '\t' ;
      (*outfiles[i]) << eas.eps2_fluid_cgs(i) << '\t' ;
      (*outfiles[i]) << eas.eps2_leak_cgs(i) << '\t' ;
      (*outfiles[i]) << eas.eps_cactus(i) << '\t' ;
      (*outfiles[i]) << F.eta[i] << '\t' ;
  ...
  auto range = EOS_Tabulated::eps_range__rho_ye(rhos[0],ye,error);
  
  std::cout <<std::endl;
  std::cout<<"rho*eps_min: "<< rhos[0]*range[0] <<std::endl;
