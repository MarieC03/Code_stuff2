    ...
    inline void calc_eas(bool recompute_emission_from_absorption=true) {
        ...
        // Inside Weakhub, everything related to micrphysics is already decided in the table
           ...
           if (EOS_Leptonic::use_muonic_eos) {
              // FIXME:: wait for extension of 4D
           ...
           // Plasma process in Weakhub does not have approximated form yet.
           ...
           // Weakhub only provides opacity for nux --> need Kirchoff for nux to find emissivity
           //if ( recompute_emission_from_absorption )  weakhub must need this to ensure detailed balance to get Q
...
void EAS<T>::kappa_ast_kappa_s__temp_rho_yle_ymu_M1(
  ...
  if ( (lrho < M1_Weakhub::logrho_min_IV) || ltemp < M1_Weakhub::logtemp_min_IV) {
  ...
  } else if (lrho > M1_Weakhub::logrho_max_IV || ltemp > M1_Weakhub::logtemp_max_IV) {
     std::cout << lrho << " " << M1_Weakhub::logrho_max_IV << " " << ltemp << " " << M1_Weakhub::logtemp_max_IV << "\n";
     CCTK_ERROR("lrho or ltemp > maximum in Weakhub table") ;
  } else if (yle > M1_Weakhub::ye_max_IV) { 
     yle = M1_Weakhub::ye_max_IV;
  } else if (yle < M1_Weakhub::ye_min_IV) { 
     yle = M1_Weakhub::ye_min_IV;
  } else if (log(ymu) > M1_Weakhub::logymu_max_IV) {
     ymu = exp(M1_Weakhub::logymu_max_IV);
  } else if (log(ymu) < M1_Weakhub::logymu_min_IV) {
