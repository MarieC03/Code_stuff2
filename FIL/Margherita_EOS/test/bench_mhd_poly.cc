...
constexpr int num_temp = 200;

//constexpr double Ye = 0.2;
//constexpr double B_tot = 1.e-3;
...
int main(int argc, char** argv) {
  ...
  auto B_tot = double {atof(argv[1])};
  auto tau_fix = bool { atoi(argv[2])};
  ...
  Cold_PWPoly::rhomax = 1.e-2;
  Cold_PWPoly::rhomin = 1.e-16;

  EOS_Polytropic::gamma_th_m1 = (gamma_th - 1.);
  EOS_Polytropic::entropy_min = (entropy_min);

  EOS_Polytropic::eos_tempmax=40.;
  EOS_Polytropic::eos_tempmin=1.e-4;
  ...
  std::uniform_real_distribution<> dist_rho(
      std::log(EOS_Polytropic::eos_rhomin), std::log(EOS_Polytropic::eos_rhomax));
  // ye uniform distrib
  std::uniform_real_distribution<> dist_ye(EOS_Polytropic::eos_yemin,
                                           EOS_Polytropic::eos_yemax);
  // temp uniform distrib
  std::uniform_real_distribution<> dist_temp(
      std::log(EOS_Polytropic::eos_tempmin),
      std::log(EOS_Polytropic::eos_tempmax));
  ...
  */
  ...
  EOS_Polytropic::atm_beta_eq = true;
  ...
  for (int nn_rho = 0; nn_rho < num_rho; nn_rho++)
    for (int nn_temp = 0; nn_temp < num_temp; nn_temp++) {
      ...
      const double B2L = METRIC.compute_square_3_gen<0,3>(Bvec);
	
      for(auto &b : Bvec) b *= B_tot/sqrt(B2L);

      EOS_Polytropic::error_type error; // TODO How do we handle this?

      PRIMS[PRESSURE] = EOS_Polytropic::press__temp_rho_ye(
          PRIMS[TEMP], PRIMS[RHOB], PRIMS[YE], error);
      PRIMS[EPS] = EOS_Polytropic::eps_csnd2_entropy__temp_rho_ye(
          PRIMS[CS2], PRIMS[ENTROPY], PRIMS[TEMP], PRIMS[RHOB], PRIMS[YE],
          error);

      // Compute conservatives
      auto CONS =
          C2P_MHD_ENTROPY<EOS_Polytropic>::compute_conservatives(PRIMS, METRIC, Bvec);
      PRIMSN[YE] = PRIMS[YE];
      PRIMSN[TEMP] = 0.0;
...
if(tau_fix)
     CONS[TAUENERGY] = -1.e10;
      ...
      auto residual= Margherita_Con2Prim<C2P_MHD_ENTROPY, EOS_Polytropic>(CONS, PRIMSN, METRIC,
                                                  error_bits);
