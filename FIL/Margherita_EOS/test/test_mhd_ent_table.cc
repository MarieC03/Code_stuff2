...
int main() {
  ...
  */
  ...
  std::array<double, C2P_MHD_ENTROPY<EOS_Tabulated>::numprims> PRIMSN;
  ...
  std::array<double, C2P_MHD_ENTROPY<EOS_Tabulated>::numcons> CONS;
  ...
  for (int i = 0; i < MAXIT; ++i) {
    ...
    CONS = C2P_MHD_ENTROPY<EOS_Tabulated>::compute_conservatives(PRIMS, METRIC,
                                                                 Bvec);
    CONS[ENTROPYSTAR] = CONS[RHOSTAR] * PRIMS[ENTROPY];
  ...
  */
    ...
    Margherita_Con2Prim<C2P_MHD_ENTROPY, EOS_Tabulated>(CONS, PRIMSN, METRIC,
                                                        error_bits);
  }
  ////////
  // Stop timing
  ...
  std::cout << "eps_min, eps_max "
            << ": " << epsrange[0] << " , " << epsrange[1] << std::endl;
