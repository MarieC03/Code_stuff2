...
int main(int argc, char** argv) {
  ...
  auto Ye = double {atof(argv[1])};
  auto B_tot = double {atof(argv[2])};
  auto tau_fix = bool { atoi(argv[3])};
  ...
  */
  ...
  for (int nn_rho = 0; nn_rho < num_rho; nn_rho++)
    for (int nn_temp = 0; nn_temp < num_temp; nn_temp++) {
      ...
      auto CONS =
          C2P_MHD<EOS_Tabulated>::compute_conservatives(PRIMS, METRIC, Bvec);
      PRIMSN[YE] = PRIMS[YE];
      PRIMSN[TEMP] = 0.0;
...
if(tau_fix)
     CONS[TAUENERGY] = -1.e10;
      ...
      auto residual= Margherita_Con2Prim<C2P_MHD, EOS_Tabulated>(CONS, PRIMSN, METRIC,
                                                  error_bits);

      auto CONSN =
          C2P_MHD<EOS_Tabulated>::compute_conservatives(PRIMSN, METRIC, Bvec);
