...
int main() {
  ...
  // Set primitives
  std::array<double, C2P_Hydro<EOS_Polytropic>::numprims> PRIMS{{
      1.234e-5, // RHOB
      0.0,      // PRESSURE
      0.678,    // ZVECX
      -0.18,    // ZVECY
  ...
  std::array<double, C2P_Hydro<EOS_Polytropic>::numprims> PRIMSN;
  ...
  std::cout << std::endl;
  std::cout << "lorentz initial: " << WL << std::endl;
  std::cout << "h initial: " << h << std::endl;
  std::cout << std::endl;

  // Declare conservatives
  std::array<double, C2P_Hydro<EOS_Polytropic>::numcons> CONS;
  ...
  for (int i = 0; i < MAXIT; ++i) {
    ...
    CONS = C2P_Hydro<EOS_Polytropic>::compute_conservatives(PRIMS, METRIC);
    ...
    residual = Margherita_Con2Prim<C2P_Hydro, EOS_Polytropic>(CONS, PRIMSN, METRIC,
                                                   error_bits);
