...
int main() {
  ...
  std::string("/zhome/academic/HLRS/xfp/xfpmusol/EOS_Tables/scollapse/BHB_lpEOS_rho234_temp180_ye60_version_1.02_20140422.h5");
  ...
  double const W = 20.0 ;
  double const beta = 1e1 ;
  
  auto temp = 100.0 ;
  auto rho  = 1.234e-04 ; 
  auto zvec = std::array<double,3>{0.18,-0.3,0.01} ;
  // Set primitives
  std::array<double, C2P_MHD<EOS_Tabulated>::numprims> PRIMS{{
    ...
    PRIMS[PRESSURE] = EOS_Tabulated::press_h_csnd2_temp_entropy__eps_rho_ye(h, PRIMS[CS2], PRIMS[TEMP], PRIMS[ENTROPY], // out all but temp (inout)
									    PRIMS[EPS], PRIMS[RHOB], PRIMS[YE], error); // in


    auto Bvec = std::array<double,3> {1.e-4, 1.e-3,
				      0.5e-7 } ; 
    ...
    res = Margherita_Con2Prim<C2P_TYPE, EOS_Tabulated>(CONS, PRIMSN, METRIC,error_bits) ;
    timestamp_t t1 = get_timestamp() ;


      std::cout << "c2p residual: " << res << std::endl ;
