...
#pragma omp parallel for
  for(int k=0;k<cctk_lsh[2];k++) for(int j=0;j<cctk_lsh[1];j++) for(int i=0;i<cctk_lsh[0];i++) {
	...
	rho[index] = std::max(rho[index],eos::c2p_rho_atm);
        ...
        //code test using tempL = c2p_temp_atm purely
        double tempL;
        if(rho[index]<=1.01*std::max(eos::c2p_rho_atm,rho_threshold)){
           tempL = eos::c2p_temp_atm;
        }
	...
	if(ID_uses_beta_equilibrium){
	  ...
	  press[index] = Pressure_Interface<eos>::press_eps_yle_ymu__beta_eq__rho_temp(
										       ...
										       , temperature[index]
										       ,error);
//code test: this is beta_eqm with Temp and then use a fake S
          //double s_local = 1;
          //double dummy;
          //eos::press_h_csnd2_temp_eps__entropy_rho_yle_ymu(
          //      dummy, dummy, temperature[index], eps[index], s_local,
          //      rho[index], Y_e[index], ymu, error);
	...
	press[index] = eos::press__temp_rho_yle_ymu(temperature[index], rho[index], Y_e[index], ymu, error);
        ...
        // code test, hot fix for atmosphere, after this ye is weird?
        if(rho[index]<=1.01*std::max(eos::c2p_rho_atm,rho_threshold)){
           eps[index] = eos::c2p_eps_atm;
        }
